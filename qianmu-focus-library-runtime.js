...
import {loadLocalChunk} from './qianmu-feature-runtime.js?v=1.59.150';
...
export function createFocusLibraryRuntime({resolveNamespace,owner,context,choices,generate,ui,notify,save,available=()=>true,watchView=()=>null}) {
  ...
  function dialogue(){return dialoguePromise||=loadLocalChunk('./qianmu-focus-dialogue.js?v=1.59.150').then(({createFocusDialogueLibrary})=>createFocusDialogueLibrary({owner,save,
  ...
  function warm(){return Promise.all([loadLocalChunk('./qianmu-focus-dialogue-ui.js?v=1.59.150'),loadLocalChunk('./qianmu-focus-dialogue.js?v=1.59.150')]);}
  ...
  async function open(options){
    ...
    try{
      ...
      if(!options?.management){
        const [{openFocusDialogue},library]=await Promise.all([loadLocalChunk('./qianmu-focus-dialogue-ui.js?v=1.59.150'),dialogue()]);await guard();
      ...
      const {openFocusLibrary}=await loadLocalChunk('./qianmu-focus-library-ui.js?v=1.59.150');await guard();
