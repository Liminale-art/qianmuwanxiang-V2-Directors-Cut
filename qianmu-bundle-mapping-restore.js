...
const sourcesOrdered=heads=>[...heads].sort((a,b)=>a.key.localeCompare(b.key)||a.createdAt-b.createdAt||a.bytes-b.bytes);
export function hasNativeMappingSources(journal){
  const flags=['listMappingSourceHeads','loadMappingSource','inspectMappingSources','importMappingSource'].map(key=>typeof journal?.[key]==='function');
  if(flags.some(Boolean)&&!flags.every(Boolean))fail('迁移来源保全接口不完整，请刷新前端');return flags.every(Boolean);
}
...
export async function planBundleMappingRestore({index,opened,journal,reservations=[],guard=async()=>{},isCurrent=()=>true}){
  ...
  const native=hasNativeMappingSources(journal),sourceBefore=native?await journal.listMappingSourceHeads(namespace,{guard,isCurrent}):null;
  ...
  for(const head of index.heads){
    if(native){
      if(sourceBefore.some(row=>sameBundleMappingHead(row,head))){
        const saved=await journal.loadMappingSource(namespace,head,{guard,isCurrent}),incoming=await opened.readJson(bundleMappingEntryId(head));await guard();
        await inspectBundleMappingReceipt(saved,head,namespace);await inspectBundleMappingReceipt(incoming,head,namespace);
        if(await digest(saved)!==await digest(incoming))fail('历史迁移凭据原文冲突，未覆盖或丢弃');
      }else{added++;addedBytes+=head.bytes;}continue;
    }
  ...
  let capacity;
  try{if(native)capacity=await journal.inspectMappingSources(namespace,index.heads,{reservations:reserved,guard,isCurrent});else validateBundleMappingHeads([...byKey.values()],namespace);}catch(_){fail('迁移凭据总空间、来源或名额不符（含本次新映射预留），未恢复；不会自动删除历史');}
  await guard();
  ...
  if(native&&await digest(sourcesOrdered(sourceBefore))!==await digest(sourcesOrdered(await journal.listMappingSourceHeads(namespace,{guard,isCurrent}))))fail('核对期间保全凭据已变化，请重新核对');
  ...
  return {...summary,digest:await digest({index:index.digest,before:ordered(before),reserved:ordered(reserved),...(native?{sources:sourcesOrdered(sourceBefore),capacity:capacity.digest}:{})})};
}
...
export async function verifyBundleMappingRestore({index,journal,guard=async()=>{},isCurrent=()=>true}){
  if(hasNativeMappingSources(journal)){
    await guard();const heads=await journal.listMappingSourceHeads(index.namespace,{guard,isCurrent});
    if(index.heads.some(head=>!heads.some(row=>sameBundleMappingHead(row,head))))fail('历史迁移凭据尚未完整保全，未继续恢复资源');await guard();return;
  }
...
export async function restoreBundleMappings({index,opened,journal,confirmed=false,guard=async()=>{},isCurrent=()=>true}){
  ...
  const native=hasNativeMappingSources(journal);
  for(const head of index.heads){
    ...
    if(native)await journal.importMappingSource(receipt,{head,confirmed:true,guard,isCurrent});else await journal.importMappingReceipt(receipt,{head,confirmed:true,isCurrent});await guard();
    const saved=native?await journal.loadMappingSource(index.namespace,head,{guard,isCurrent}):await journal.loadMappingReceipt(index.namespace,head.kind,head.digest,{isCurrent});
