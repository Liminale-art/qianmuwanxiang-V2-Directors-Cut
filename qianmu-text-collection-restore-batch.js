...
import {partitionTextCollectionMutations} from './qianmu-text-collection-bulk-contract.js';
...
export function createTextCollectionRestoreBatch({backup,session,check,confirm,onProgress=()=>{},uid=notesSyncOperationId}={}){
  if(!session||typeof session.guard!=='function'||typeof session.batchInfo!=='function'||typeof session.prepareBatch!=='function'||typeof session.prepareRestore!=='function'
  ...
  const controller=new AbortController();let confirmed=0,authorized=false,closed=false,pending=null,uncertain=false,batches=null,batchIndex=0;
  ...
  async function execute(){
    ...
    if(!authorized){
      ...
      batches=partitionTextCollectionMutations(operations.map(operation=>operation.request),{expectedAccount:session.expectedAccount,maxItems:info.maxItems,maxBytes:info.maxBytes}).map(items=>session.prepareBatch(items));
    ...
    while(confirmed<operations.length){
      ...
      confirmed+=batch.request.mutations.length;batchIndex++;uncertain=false;
