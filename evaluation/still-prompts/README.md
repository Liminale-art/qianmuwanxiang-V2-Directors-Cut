...
厨房六镜保留原始六镜目标。v368维护节点曾发现生产合同仅支持四镜；v369将明确选择的数量区间扩展至六镜，两阶段合同、流式/提交额度与旧镜头读取保持一致。语料现按六镜进入真实合同，不用四镜替代。默认仍每层最多三张、同时生成两张；旧v1数量设置不会因升级扩大，只有明确选择更大范围或完整校验的v2导入可启用五至六镜。工具仍保留 `unsupported_shot_range` 防止未来要求被当前合同静默缩减，不将实现缺口算作模型失败。
