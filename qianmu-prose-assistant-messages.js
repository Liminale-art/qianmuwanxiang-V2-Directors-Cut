...
export function compileProseAssistantMessages({context,question,systemPrompt}={}){
  if(!text(systemPrompt,20000))fail('正文助手系统提示词尚未配置，未使用其他模块提示词兜底');
  if(!text(question,20000)||typeof context?.assertCurrent!=='function')fail('正文助手问题或引用未就绪');context.assertCurrent();
  const ref=context.reference;
  if(!ref||!floor(ref.floor)||!['selection','floor'].includes(ref.mode)||!text(ref.text,PROSE_ASSISTANT_SOURCE_LIMIT))fail('正文助手引用格式无效');
  if(!Array.isArray(context.previous)||context.previous.length>limits.maxPreviousFloors||!Array.isArray(context.history)||context.history.length>limits.historyPairs)fail('正文助手上下文范围无效');
  let previousFloor=-1,previousCharacters=0,historyCharacters=0;
  const previous=context.previous.map(row=>{
    if(!row||!floor(row.floor)||row.floor<=previousFloor||row.floor>=ref.floor||!['user','character'].includes(row.role)||!text(row.text,limits.previousCharacters)||(previousCharacters+=row.text.length)>limits.previousCharacters)fail('正文助手参考前文顺序或容量无效');
    previousFloor=row.floor;return {floor:row.floor,speaker:row.role,text:row.text};
  });
  const messages=[{role:'system',content:systemPrompt}];
  for(const pair of context.history){
    if(!pair||!text(pair.user,limits.historyCharacters)||!text(pair.assistant,limits.historyCharacters)||(historyCharacters+=pair.user.length+pair.assistant.length)>limits.historyCharacters)fail('正文助手近期问答无效');
  ...
  messages.push({role:'user',content:JSON.stringify({type:'qianmu-prose-assistant-input',version:1,
    reference:{floor:ref.floor,mode:ref.mode,text:ref.text},previous,question})});
  if(messages.reduce((sum,row)=>sum+row.content.length,0)>300000)fail('正文助手本次消息过长，请缩小引用范围');
