...
export function createVibeReviewController({actions,onAdd,onClose,confirm=async()=>false,onNotice=()=>{},icons=()=>{},isCurrent=()=>true,createPreservation=null}){
  ...
  function render(){
    ...
    host.querySelector('.sd-vibe-review').setAttribute('aria-busy',String(busy));
    if(loaded&&!busy&&!message&&!displayed.length&&!archive)host.querySelector('.sd-vibe-review-rows').innerHTML=`<p class="sd-vibe-empty" role="status">${historical?'暂无历史编码记录':'暂无当前编码记录'}</p>`;
