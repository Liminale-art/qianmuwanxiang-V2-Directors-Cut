...
import {createComfySceneJournal,captureComfySceneJournalSelection} from './qianmu-comfy-scene-journal.js';
...
export function createNativeComfySceneStore({legacy,createStorage,indexedDB=globalThis.indexedDB,journal=createComfySceneJournal({indexedDB}),now=Date.now}={}){
  ...
  const journalSelect=(namespace,query)=>{if(typeof journal.select==='function')return journal.select(namespace,query);const select=captureComfySceneJournalSelection(namespace,query);return Promise.resolve(journal.read(namespace)).then(select);};
  const journalControl=namespace=>journalSelect(namespace,{kind:'control'});
  const journalClaims=(namespace,filter={})=>journalSelect(namespace,{kind:'claims',scope:null,chatKey:null,pendingOnly:false,...filter});
  const journalAuthority=receipt=>journalSelect(receipt.scope.namespace,{kind:'authority',receipt});
  ...
  const localOwners=async state=>{
    const claims=await journalClaims(state.scope.namespace,{scope:state.scope}),holders=(state.record?.holders||[]).filter(row=>row.status!=='uncertain');
  ...
  const run=(namespace,options,work)=>{namespace=assertComfyRouteNamespace(namespace);const captured={...options};
    ...
    const task=queue.then(async()=>{await check();const local=await journalControl(namespace),opts={guard:check,signal:captured.signal,requireExisting:local.nativeKnown,inventoryOnly:captured.inventoryOnly===true};
  ...
  const clear=(namespace,chatKey,options={})=>run(namespace,options,async opts=>{
    await deliver(namespace,opts,{chatKey});const current=await catalogue.all(namespace,{...opts,...(chatKey===null?{}:{chatKey})});if(options.expectedGeneration!==current.generation)fail('续场清理代数已变化');
  ...
  return Object.freeze({
    ...
    review(namespace,chatKey,options={}){return run(namespace,{...options,reviewOnly:true},async opts=>{
      const state=await catalogue.review(namespace,chatKey,opts),local=await journalSelect(namespace,{kind:'summary'});
      return {...state,native:true,local};
    });},
    synchronize(namespace,options={}){return run(namespace,options,async opts=>{
      const scopes=new Map((await journalClaims(namespace,{pendingOnly:true})).map(row=>[comfySceneScopeKey(row.receipt.scope),row.receipt.scope])),errors=[];
      for(const scope of scopes.values())try{await deliver(namespace,opts,{scope});}catch(error){await opts.guard();if((await journalControl(namespace)).pending)throw error;errors.push({scope,error:String(error?.message||'原结果暂不可保存').slice(0,300)});}
      return {errors};
    });},
    resolveSource(scope,{heads,generation,selected,acknowledged},options={}){
      ...
      return run(scope.namespace,options,async opts=>{await deliver(scope.namespace,opts,{scope});const state=await catalogue.reviewScope(scope,opts);
    ...
    async settle(receipt,outcome){const captured=normalizeComfySceneReceipt(receipt);await journal.remember(captured,outcome);
      return run(captured.scope.namespace,{},async opts=>{await deliver(captured.scope.namespace,opts,{scope:captured.scope});return {view:resultView(await catalogue.reviewScope(captured.scope,opts))};});},
    ...
    resultSnapshot(scope,options={}){scope=comfySceneScope(scope);return run(scope.namespace,options,async opts=>{await deliver(scope.namespace,opts,{scope});const state=await catalogue.reviewScope(scope,opts);return {...resultView(state),heads:state.heads};});},
    confirmResult(scope,request,options={}){scope=comfySceneScope(scope);const captured=structuredClone(request),operation=captureComfySceneAction({...captured,type:'confirm_result'},scope);
      return run(scope.namespace,options,async opts=>{await deliver(scope.namespace,opts,{scope});const state=await catalogue.reviewScope(scope,opts),summary=resultView(state);
    ...
    list(namespace,chatKey){return run(namespace,{},async opts=>{await deliver(namespace,opts,{chatKey});return (await catalogue.all(namespace,{...opts,chatKey})).rows.map(view);});},
    chatUsage(namespace,chatKey){return run(namespace,{},async opts=>{await deliver(namespace,opts,{chatKey});const value=await catalogue.all(namespace,{...opts,chatKey});return {count:value.rows.length,bytes:value.rows.reduce((n,row)=>n+sceneBytes(row.record),0),generation:value.generation};});},
