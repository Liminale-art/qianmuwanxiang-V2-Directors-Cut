...
function metadata(head,formatBytes){
  return `<section class="sd-mapping-meta">${pair('首次确认',new Date(head.createdAt).toLocaleString())}${pair('原包指纹',head.sourceDigest)}${pair('聊天指纹',head.chatHash)}${pair('凭据指纹',head.digest)}${pair('记录内容',formatBytes(head.bytes))}</section>`;
}
export function renderMappingRegistry({list,detail,busy,notice,query,kind},formatBytes){
  ...
  const items=detail?detail.rows.map(row=>detail.head.kind==='environment'?`<section class="sd-mapping-item">${pair('来源安装',row.sourceInstance)}${pair('来源账户标记',row.sourceAccount)}${pair('目标安装',row.targetInstance)}${pair('目标账户标记',row.targetAccount)}</section>`
    :`<section class="sd-mapping-item"><b>${row.source.category.toUpperCase()} · ${row.source.scope==='default'?'默认绑定':'指定聊天'}</b>${pair('调整前',row.source.subjectKey)}${pair('调整后',row.target.subjectKey)}${pair('角色档案',row.source.archiveId||'不使用档案（显式解除）')}${row.source.scope==='chat'?pair('原绑定聊天',row.source.chatKey):''}${pair('原版本',row.source.revision)}${pair('派生版本',row.target.revision)}${pair('原资料摘要',row.sourceHash||'未取得原资料')}${pair('目标资料摘要',row.targetHash)}</section>`).join('')
    :(list?.rows||[]).map((head,index)=>`<button type="button" class="sd-btn sd-mapping-item" data-mapping-open="${index}" ${busy?'disabled':''}><b>${name(head.kind)}</b><small>${stamp(head.createdAt)} · ${escape(formatBytes(head.bytes))}</small><small>原包 ${head.sourceDigest.slice(0,12)} · ${head.mappings} 项${head.bindings?` / ${head.bindings} 条绑定`:''}</small><small>聊天 ${head.chatHash.slice(0,12)}</small></button>`).join('');
