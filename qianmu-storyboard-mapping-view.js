...
export function openMappingRegistry({parent,run,icons=()=>{},formatBytes}){
  ...
  let closed=false,resolve,listScroll=0,downloadUrl=null,downloadTimer,importFile=null,focusField=null,listFocus=null;
  ...
  const close=()=>{if(closed)return;closed=true;controller.abort();importFile=null;state.importReview=null;releaseDownload();if(dialog.open)dialog.close();dialog.remove();if(returnFocus?.isConnected)returnFocus.focus({preventScroll:true});resolve();};
  ...
  const captureFocus=()=>{
    ...
    const field=element&&[...element.attributes].find(attribute=>attribute.name.startsWith('data-mapping-'));
    return field?{name:field.name,value:field.value,selection:element.matches('[data-mapping-query]')?[element.selectionStart,element.selectionEnd,element.selectionDirection]:null}:null;
  };
  const draw=(scroll=dialog.querySelector('main')?.scrollTop||0,preferredFocus=null)=>{
    if(!active())return;focusField=preferredFocus||captureFocus()||focusField;
    dialog.innerHTML=renderMappingRegistry(state,formatBytes);icons(dialog);dialog.querySelector('main').scrollTop=scroll;
    if(!state.busy&&focusField){
      const next=[...dialog.querySelectorAll('button,input,select')].find(node=>node.getAttribute(focusField.name)===focusField.value&&!node.matches(':disabled'))
        ||dialog.querySelector(state.importing?'[data-mapping-action="import-back"]':state.detail?'[data-mapping-action="back"]':'[data-mapping-query]');
      next?.focus({preventScroll:true});if(next?.matches('[data-mapping-query]')&&focusField.selection)next.setSelectionRange(...focusField.selection);
    }
  };
  const request=async(action,input)=>{if(!active())throw Error('迁移凭据页面已关闭');const result=await run(action,{input,signal:controller.signal});if(!active())throw Error('迁移凭据页面已关闭');return result;};
  ...
  async function work(action,input,scroll=0){
    if(!active()||state.busy)return;remember();state.busy=true;state.notice='';if(action.startsWith('mapping-import-'))state.importConfirmed=false;if(action==='mapping-list')state.list=null;draw();
    try{
      ...
      if(action==='mapping-export'){
        ...
        importFile=null;state.importing=false;state.importReview=null;state.detail=null;state.list=null;state.kind='all';state.query=result.head.digest;
        state.notice=result.outcome==='added'?'已保全完整历史凭据；原时间和关系保留，当前绑定未修改。':'原记录完全相同，已重新核对；未新增或覆盖。';
        state.list=await request('mapping-list',{kind:state.kind,query:state.query,offset:0});
    ...
    }catch(error){if(!active())return;if(action.startsWith('mapping-import-')){state.importReview=null;state.importConfirmed=false;}state.notice=[state.notice,error?.message||'迁移凭据核对失败，请保留原记录'].filter(Boolean).join(' ');}
  ...
  const chooseFile=()=>{
    if(!active()||state.busy)return;listFocus={name:'data-mapping-action',value:'import'};
    ...
    input.addEventListener('cancel',event=>{event.stopPropagation();input.remove();},{once:true});input.addEventListener('change',()=>{
      const current=input.isConnected&&active()&&!state.busy,file=input.files?.[0];input.remove();if(!file||!current)return;listScroll=dialog.querySelector('main')?.scrollTop||0;importFile=file;state.importing=true;state.importReview=null;state.importConfirmed=false;state.fileName=file.name;
      void work('mapping-import-preview',{file});
    },{once:true});input.click();
  };
  dialog.addEventListener('cancel',event=>{if(event.target===dialog){event.preventDefault();close();}});dialog.addEventListener('close',close);
  ...
  dialog.addEventListener('change',event=>{if(active()&&!state.busy&&event.target.matches('[data-mapping-import-confirm]')){state.importConfirmed=event.target.checked;draw();}});
  dialog.addEventListener('click',event=>{
    const action=event.target.closest('[data-mapping-action]')?.dataset.mappingAction;
    if(action==='close'){close();return;}if(!active()||state.busy)return;
    if(action==='import'){chooseFile();return;}
    if(action==='import-back'){importFile=null;state.importing=false;state.importReview=null;state.importConfirmed=false;state.notice='';draw(listScroll,listFocus);return;}
    ...
    if(open){const head=state.list?.rows[Number(open.dataset.mappingOpen)];if(head){listScroll=dialog.querySelector('main').scrollTop;listFocus={name:'data-mapping-open',value:open.dataset.mappingOpen};void work('mapping-detail',{kind:head.kind,digest:head.digest,offset:0});}return;}
    if(action==='back'){state.detail=null;state.notice='';draw(listScroll,listFocus);return;}
