...
import { describeRunningHubUsage } from './qianmu-runninghub-usage.js';
...
export function mountComfyInbox(host, { service, receive, isCurrent = () => host.isConnected } = {}) {
  ...
  function paint() {
    ...
    const values = [total.imageBytes,total.metadataBytes,total.temporaryBytes];
    const storedBytes = values.every(value => Number.isFinite(value) && value >= 0) ? values.reduce((sum,value) => sum + value,0) : null;
    ...
    host.innerHTML = `<section class="sd-comfy-inbox" aria-label="Comfy 收片管理" aria-busy="${busy}">
      ...
      ${server?.cloudCapabilities ? `<p class="sd-comfy-inbox-capabilities">${[['comfy-cloud','Comfy Cloud'],['runninghub','RunningHub']].map(([id,label]) => `${label} · ${server.cloudCapabilities.resultRetrieval && server.cloudCapabilities.resultProviders.includes(id) ? '可领取原图' : '暂未开放收图'}`).join('；')}${server.cloudCapabilities.submission ? `。新任务支持 ${(server.cloudCapabilities.submissionProviders ?? ['comfy-cloud']).map(id=>id==='runninghub'?'RunningHub':'Comfy Cloud 主站').join(' / ')}手动文本工作流` : '。云端新任务暂未开放'}</p>` : ''}
  ...
  async function refresh() {
    ...
    const results = await Promise.allSettled([service.list(), service.catalogAll ? service.catalogAll() : service.catalog()]);
    ...
    serverError = results[1].status === 'rejected' ? results[1].reason?.message || 'Comfy 目录读取失败' : server?.warning || '';
