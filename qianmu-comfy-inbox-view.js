...
export function mountComfyInbox(host, { service, receive, isCurrent = () => host.isConnected } = {}) {
  ...
  function paint() {
    ...
    host.innerHTML = `<section class="sd-comfy-inbox" aria-label="Comfy 收片管理" aria-busy="${busy}">
      ...
      ${server?.cloudCapabilities ? `<p class="sd-comfy-inbox-capabilities">${[['comfy-cloud','Comfy Cloud'],['runninghub','RunningHub']].map(([id,label]) => `${label} · ${server.cloudCapabilities.resultRetrieval && server.cloudCapabilities.resultProviders.includes(id) ? '可领取原图' : '暂未开放收图'}`).join('；')}${server.cloudCapabilities.submission ? '。新任务仅支持 Comfy Cloud 主站手动文本工作流' : '。云端新任务暂未开放'}</p>` : ''}
