...
export function mountStoryboardTagComplete(root,{items=()=>[],source=()=> 'novel',format=()=> 'tags',isCurrent=()=>true,onUse=()=>{},onNotice=()=>{}}={}){
  ...
  let field=null,popup=null,mirror=null,observer=null,frame=0,timer=0,composing=false,disposed=false,press=null,suppressClick=0,accepting=false;
  let index=[],rows=[],total=0,offset=0,active=0,snapshot=null,previousAttributes=null;
  ...
  function position(){
    ...
    mirror.style.boxSizing='border-box';mirror.style.height='auto';
    mirror.style.left=`${rect.left}px`;mirror.style.top=`${rect.top}px`;mirror.style.width=`${field.clientWidth+parseFloat(style.borderLeftWidth)+parseFloat(style.borderRightWidth)}px`;
    mirror.textContent=field.value.slice(0,field.selectionStart);const marker=doc.createElement('span');marker.textContent=field.value.slice(field.selectionStart)||'\u200b';mirror.append(marker);
    const caret=marker.getClientRects()[0]||marker.getBoundingClientRect(),width=Math.min(360,Math.max(0,view.width-16));popup.style.width=`${width}px`;
  ...
  function show(){
    ...
    popup.addEventListener('pointerdown',event=>{const button=event.target.closest('button');if(!button)return;win.clearTimeout(timer);timer=0;press={button,x:event.clientX,y:event.clientY,moved:false};if(event.pointerType==='mouse')event.preventDefault();});
  ...
  function refresh(){
    win.clearTimeout(timer);timer=0;if(press||accepting)return;if(composing||!current()||!field){hide();return;}
    const fragment=storyboardTagFragment(field.value,field.selectionStart,field.selectionEnd,{format:format(field)});
    if(!fragment.query.trim()||fragment.query.length>120){hide();return;}
    const result=searchStoryboardTags(index,{query:fragment.query,offset,limit:40});rows=result.rows;total=result.total;
  ...
  function accept(choice){
    ...
    const target=field;accepting=true;
    try{hide();target.focus({preventScroll:true});target.setRangeText(result.insert,result.replaceStart,result.replaceEnd,'end');target.setSelectionRange(result.start,result.end);
      target.dispatchEvent(new win.Event('input',{bubbles:true}));onUse(live);
    }finally{accepting=false;}
  }
  function focus(target){
    if(accepting||!selectedField(target)||!current())return;if(field!==target){hide();field=target;offset=0;index=createStoryboardTagIndex(items(),source(field));}refresh();
  }
  const schedule=()=>{win.clearTimeout(timer);offset=0;timer=win.setTimeout(refresh,65);};
  on(root,'focusin',event=>focus(event.target));on(root,'input',event=>{if(!accepting&&selectedField(event.target)){if(field!==event.target)focus(event.target);else if(!composing)schedule();}});
  ...
  on(doc,'selectionchange',()=>{if(popup&&!press&&!accepting&&doc.activeElement===field&&!composing)schedule();});
