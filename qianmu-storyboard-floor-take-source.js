import {createStoryboardMessageReference} from './qianmu-storyboard.js?v=1.59.233';
import {createStoryboardStreamLineage} from './qianmu-storyboard-stream-lineage.js?v=1.59.233';
import {readStoryboardContinuationLinks,storyboardContinuationSavePending} from './qianmu-storyboard-continuation-proof.js?v=1.59.233';
import {storyboardStreamDigest,storyboardStreamFingerprint,storyboardStreamGeneration} from './qianmu-storyboard-stream-reference.js?v=1.59.233';
...
export async function readStoryboardFloorTakeSourceKeys(floor,message,api,namespace,current){
  ...
  for(const link of links){
    ...
    check();if(await storyboardStreamDigest(JSON.stringify([link.namespace,link.chatKey,link.from,link.to,link.digest]))!==link.id)fail();check();
  }
