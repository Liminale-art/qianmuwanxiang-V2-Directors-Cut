...
export function createProseAssistantSession({key,isCurrent,onChange=()=>{},initialHistory}={}){
  if(typeof key!=='string'||!key||key.length>4096||typeof isCurrent!=='function'||typeof onChange!=='function')fail('prose_assistant_setup','场外特助会话来源未就绪');
  ...
  async function run({question,source,request}={}){
    ...
    try{
      ...
      if(token.context.reference){const {floor,replyId,mode,range}=token.context.reference;row.reference=reference({floor,replyId,mode,range});}
