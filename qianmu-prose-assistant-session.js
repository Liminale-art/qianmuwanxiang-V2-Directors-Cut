...
export const PROSE_ASSISTANT_SESSION_LIMITS=Object.freeze({turns:100,characters:1000000,question:20000,reply:200000});
...
export function createProseAssistantSession({key,isCurrent,onChange=()=>{}}={}){
  ...
  const limits=PROSE_ASSISTANT_SESSION_LIMITS,rows=[];let active=null,closed=false,characters=0,nextId=0;
  ...
  async function run({question,source,request}={}){
    ...
    if(rows.length>=limits.turns||characters+question.length>=limits.characters)fail('prose_assistant_capacity','助手会话已达本机容量上限，请先复制需要的内容，再清空会话');
    const prior=history(),row={id:++nextId,user:question,assistant:'',status:'running'},token={row,controller:new AbortController(),context:null};
  ...
  return Object.freeze({key,run,history,view(){checkLive();return snapshot();},stop,clear(){checkLive();stop();rows.length=0;characters=0;publish();},close});
}
