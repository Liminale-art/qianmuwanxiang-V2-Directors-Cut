...
export function validateVibeStorageSummary(value,namespace){
  const native=[3,4].includes(value?.version),files=value?.version===4;
  if(!account(namespace)||![2,3,4].includes(value?.version)||value.namespace!==namespace||value.status!=='ready'||Object.keys(value).some(key=>!['version','status','namespace','bytes','assets','previews','records','metadata',...(native?['persistence','retained']:[]),...(files?['feeOriginals']:[])].includes(key)))throw unavailable('Vibe 计值版本或归属无效，请刷新后重读');
  ...
  const feeOriginals=files?validateVibeReceiptFileUsage(value.feeOriginals,records.count):null;
  return {version:value.version,status:'ready',namespace,bytes:value.bytes,metadata:{...metadata},...(native?{persistence:value.persistence,retained:{...value.retained},...(feeOriginals?{feeOriginals}:{})}:{}),
