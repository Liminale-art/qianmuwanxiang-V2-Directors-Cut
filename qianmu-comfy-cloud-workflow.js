...
export function bindComfyCloudWorkflow(workflowJson, input, uploads) {
  return prepareComfyCloudWorkflow(workflowJson, input).bind(uploads);
}

// Preview is for pre-upload admission only, never an executable task/receipt.
export function prepareComfyCloudWorkflow(workflowJson, input) {
  ...
  if(!Array.isArray(input?.references)||!input.references.length||input.references.length>COMFY_REFERENCE_LIMIT)fail();
  const template=prepareComfyWorkflow(original,{...input,referenceCount:input.references.length});
  function bindValues(references) {
  ...
  return {preview:bindValues(input.references.map((_,index)=>`qianmu-preview-${index}.png`)),bind(uploads){
    if(!Array.isArray(uploads)||uploads.length!==input.references.length)fail();
    return bindValues(uploads.map((receipt,index)=>matchComfyCloudUpload(receipt,input.connection,input.references[index])));
  }};
}
