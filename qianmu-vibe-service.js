...
export function createVibeEncodingService({dataRoot,store=createImageServiceStore({dataRoot,scope:'vibe'}),cache,
  ...
  const packet=(result,stored=true,warning='')=>({ok:true,version:1,result,stored,...(warning?{warning}:{})});
  async function retrieve(req,value,row){
    ...
    if(!result)throw fail('pending','原 Vibe 编码尚未暂存或已被清理；未重新编码','unknown');return packet(result);
  }
  return {
    ...
    async submit(req,input,{signal}={}){
      ...
      try{
        ...
        if(previous){
          const saved=await results.load(owner(value,previous));check(req,value);if(saved)return packet(saved);
        ...
        const work=queue.run({apiKey:resource(value.namespace,value.cacheKey),namespace:value.namespace,attemptId,requestDigest:value.cacheKey,requestBytes:weight,
          ...
          let authorized=false,result;
          try{
            // Disconnect stops waiting/preparation, not a POST that might have charged. Completed bytes are still cached for this owner.
            result=await encode(captured,{...gatewayOptions,guard:async()=>{check(req,value);if(closed||signal?.aborted)throw fail('cancelled','编码等待已取消，未继续提交');},
              authorize:async(actual,key)=>{if(key!==prepared.cacheKey||JSON.stringify(actual)!==JSON.stringify(prepared.identity))throw fail('identity','编码参数已变化');await ticket.beforeSubmit();authorized=true;return true;}});
          ...
          return packet(result,stored,stored?'':'编码已返回；服务暂存未完成，请先保存在当前设备，勿重复提交');
        });jobs.set(id,work);
    ...
    async close(){closed=true;queue.close();await Promise.allSettled([...jobs.values()]);await store.close();},
