...
export function createNativeTextCollectionClient({expectedAccount,guard,headers,storageFactory=createConfiguredStAccountStorage,legacyFactory=createTextCollectionClient,now=Date.now}={}) {
  let closed=false,opening=null,storage=null;const legacy=legacyFactory({expectedAccount,guard,headers});
  const check=async()=>{if(closed||await guard()===false||closed)throw error('account','收藏账户或页面已变化',401);return true;};
  ...
  async function open(options){
    await check();if(storage)return storage;if(opening)return opening;
    opening=(async()=>{
      const candidate=await storageFactory({maxBytes:limits.bytes,isCurrent:()=>!closed});
      try{
        const current=await candidate.read('collections',{...options,guard:check});await check();
        if(!current.exists){
          ...
          await candidate.write('collections',validate(initial),{...options,expectedFingerprint:null,guard:check});
        }else validate(current.value);
        await check();storage=candidate;return storage;
  ...
  async function read(options){const store=await open(options);const result=await store.read('collections',{...options,guard:check});await check();if(!result.exists)throw error('missing','收藏资料已变化，请重新打开');return validate(result.value);}
  async function mutate(inputs,options){
    ...
    await store.update('collections',value=>{
    ...
    },{...options,guard:check});await check();return acknowledgements;
  }
  async function query(method,input={},options){
    ...
    const state=await read(options),common=base(state),remainingRecords=limits.records-state.entries.length,remainingMutations=limits.mutations-state.revision;let response;
  ...
  return Object.freeze({persistence:'st-account-file',concurrency:'optimistic-non-cas',
    ...
    close(){closed=true;legacy.close();storage?.close();}});
}
