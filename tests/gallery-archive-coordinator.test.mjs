...
test('actual entry completion schedules even an already stripped gallery, but never after epoch change',async()=>{
  let scheduled=0;const context=vm.createContext({migrateGallerySnapshots,storyboardRecipeRecordMetadata,storyboardSnapshotArchiveBusy:0,storyboardSnapshotEpoch:1,storyboardGalleryRecords:()=>[],getChatKey:()=> 'chat',
