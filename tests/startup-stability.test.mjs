...
assert.equal(manifest.version, '1.45.5');
...
assert.match(source, /const VERSION = '1\.45\.5';/);
...
const first = createGuard(Symbol('first'), 'index.js?v=1.45.5');
const duplicate = createGuard(Symbol('duplicate'), 'index.js?v=1.45.5&copy=2');
...
assert.match(source, /qianmu-icon-renderer\.js\?v=1\.45\.5/, '稳定版必须按版本加载局部图标渲染器');
