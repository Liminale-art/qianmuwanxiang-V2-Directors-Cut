...
assert.equal(manifest.version, '1.50.0');
...
assert.match(source, /const VERSION = '1\.50\.0';/);
...
const first = createGuard(Symbol('first'), 'index.js?v=1.50.0');
const duplicate = createGuard(Symbol('duplicate'), 'index.js?v=1.50.0&copy=2');
...
assert.match(source, /qianmu-icon-renderer\.js\?v=1\.50\.0/, '稳定版必须按版本加载局部图标渲染器');
