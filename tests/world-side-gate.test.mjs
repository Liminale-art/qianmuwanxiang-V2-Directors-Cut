...
test('world-side shots are an explicit default-off storyboard setting', () => {
  ...
  assert.deepEqual(createStoryboardDefaults().directorBridge, { worldSideShotsEnabled: false,worldAutoGenerate:false,worldAutoMaxImages:1 });
  const migrated = normalizeStoryboardState({ schemaVersion: 23, directorBridge: { worldSideShotsEnabled: true } });
  assert.deepEqual(migrated.directorBridge, { worldSideShotsEnabled: false,worldAutoGenerate:false,worldAutoMaxImages:1 }, 'upgrades require a fresh explicit opt-in');
