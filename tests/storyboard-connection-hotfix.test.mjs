...
assert.match(source, /import\('\.\/qianmu-image-direct\.js\?v=1\.59\.135'\)/, '生图渠道必须按需接入浏览器直连适配层');
...
assert.match(source, /directImage\.generateDirectImage\(gatewayRequest, \{ probeTransport: true, beforeSubmit \}\)/, '正式生图先做只读连接探测，提交前核对取消；已提交的网络错误不得自动转发重试');
