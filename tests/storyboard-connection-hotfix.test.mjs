...
assert.match(source, /checkDirectImageConnection, generateDirectImage, isDirectImageTransportError/, 'NAI 必须接入浏览器直连适配层');
...
assert.match(source, /async function storyboardLoadConnectionPreset[\s\S]*storyboardResolveApiKey\(sourceId, preset\.credentialId\)[\s\S]*已载入 API 预设/, '选择 API 预设必须同时载入连接和对应凭据');
assert.match(source, /value="\$\{htmlEscape\(storyboardDraftApiKeys\.get\(state\.source\) \|\| ''\)\}"/, '表单重绘后必须恢复失败的 Key 输入');
assert.match(source, /try \{ data = await generateDirectImage\(gatewayRequest\); \}/, '正式生图必须优先使用浏览器直连适配层');
