...
const centerStatus = centerView.slice(centerView.indexOf('function renderCoreadCenterStatusView'), centerView.indexOf('function renderCoreadSpoilerGuardView'));
const spoilerGuard = centerView.slice(centerView.indexOf('function renderCoreadSpoilerGuardView'), centerView.indexOf('function renderCoreadGuideView'));
assert.doesNotMatch(centerStatus, /sd-reader-spoiler-filter/, '防剧透开关不应继续挤在全局总览卡里');
assert.ok(spoilerGuard.includes('switchHtml') && spoilerGuard.includes('防全知剧透'), '防全知剧透必须保留独立开关');
assert.match(source, /return renderCoreadSpoilerGuardView\(\{on, switchHtml: renderMemSwitch\('sd-reader-spoiler-filter', on\)\}\)/, '独立卡必须接入原有防剧透开关');
