...
import {captureStoryboardStreamFrame,createStoryboardStreamMessageReference} from '../qianmu-storyboard-stream-source.js?v=1.59.242';
import {beginStoryboardStreamAttempt as begin,normalizeStoryboardStreamAttempt as normalize,assertStoryboardStreamAttemptSettled as settled} from '../qianmu-storyboard-stream-attempt.js?v=1.59.242';
...
async function fixture(){
  ...
  let live=true,saves=0,seq=0,save=()=>saves++;
  ...
  return{state,message,host,d,reference,get saves(){return saves;},set live(value){live=value;},set save(value){save=value;},begin:async()=>begin(await reference(),null,d)};
}
...
test('a source-bound checkpoint exists before model work and contains no prose or credentials',async()=>{
  ...
  assert.throws(()=>settled(attempt.plan));assert.equal(attempt.finish('waiting'),'waiting');settled(attempt.plan);assert.equal(attempt.plan.status,'idle');
});
...
test('normalization preserves interrupted and failed markers and they cannot authorize automatic final capture',async()=>{
  for(const status of ['preparing','failed','cancelled']){
    const f=await fixture(),attempt=await f.begin();if(status!=='preparing')attempt.finish(status);
...
test('a successful checkpoint accepts only new stable content and caps partial rounds while allowing a final handoff',async()=>{
  const f=await fixture();for(let i=0;i<3;i++){
    if(i)f.message.mes+=`Later ${i}.\n\n`;const attempt=await f.begin();attempt.finish('waiting');assert.equal(attempt.plan.streamAttempt.passes,i+1);
  }
...
test('repeated unchanged content cannot reuse a completed extraction or reset its counter',async()=>{
  const f=await fixture(),attempt=await f.begin();attempt.finish('ready');await assert.rejects(f.begin(),error=>error.code==='storyboard_stream_wait');
...
test('changing an earlier waiting fragment cannot hide behind an unchanged initial source prefix',async()=>{
  const f=await fixture();(await f.begin()).finish('waiting');f.message.mes+='Second fragment.\n\n';(await f.begin()).finish('waiting');
...
test('a shortened prior prefix fails before starting another checkpoint',async()=>{
  const f=await fixture();(await f.begin()).finish('waiting');f.message.mes='Short.\n\n';await assert.rejects(f.begin(),/片段改变/);assert.equal(f.saves,2);
});
...
test('a missing, duplicated or manually controlled root plan cannot be claimed as a new partial attempt',async()=>{
  for(const kind of ['duplicate','manual','cancelled','locked','review']){
    const f=await fixture(),attempt=await f.begin();attempt.finish('waiting');f.message.mes+='New.\n\n';
...
test('completion cannot overwrite a replacement checkpoint or save through a changed source',async()=>{
  ...
  assert.equal(attempt.finish('ready'),'cancelled');assert.equal(attempt.plan.streamAttempt.requestId,'replacement');assert.equal(f.saves,1);
  const g=await fixture(),running=await g.begin();g.live=false;assert.equal(running.finish('ready'),'cancelled');assert.equal(g.saves,1);assert.equal(running.plan.streamAttempt.status,'preparing');
});
...
test('checkpoint finish preserves already queued image rows after a later partial failure',async()=>{
  ...
  attempt.finish('failed');assert.equal(attempt.plan.status,'queued');assert.equal(attempt.plan.shots[0].status,'queued');assert.equal(attempt.plan.streamAttempt.status,'failed');
  assert.throws(()=>settled(attempt.plan));attempt.finish('ready');assert.equal(attempt.plan.streamAttempt.status,'failed');
});
