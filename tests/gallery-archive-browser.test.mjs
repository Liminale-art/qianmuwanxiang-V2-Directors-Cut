...
test('actual discovery, native version/page/record readers work with no original chat and never write',async t=>{
  ...
  assert.deepEqual(await f.unchanged(),before);assert.deepEqual(Object.keys(f.s).sort(),['close','evidence','isClosed','list','open','page','preview','recipe','review','supplement']);
});
