...
assert.equal(manifest.version, '1.58.13');
...
assert.ok(header.indexOf('一蝶振翅') < header.indexOf('<h2>${EXTENSION_NAME}</h2>'));
assert.ok(header.indexOf('<h2>${EXTENSION_NAME}</h2>') < header.indexOf('sd-version-line'));
assert.match(header, /sd-version-new-dot/, 'a locally installed new version must expose a dismissible update dot');
...
console.log('V1.58.13 visual refinement contract OK');
