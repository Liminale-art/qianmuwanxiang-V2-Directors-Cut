...
const environment = { qianmuVersion: '1.59.365', backendVersion: '1.2.3', backendStatus: 'ready' };
...
test('diagnostic exclusions update preview and copy together', async () => {
    ...
    assert.match(f.copied[0], /无（未附带诊断）/); assert.doesNotMatch(f.copied[0], /Firefox|Windows|1.59.365/); f.dispose();
});
