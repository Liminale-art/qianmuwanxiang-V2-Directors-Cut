...
assert.match(source, /createFeatureRuntime\(\{[\s\S]*imageDirect:[\s\S]*import\('\.\/qianmu-image-direct\.js\?v=1\.59\.139'\)[\s\S]*featureRuntime\.load\('imageDirect'\)/, 'the direct image runtime must enter the shared on-demand feature boundary');
assert.match(source, /optionalService:[\s\S]*import\('\.\/qianmu-service-capabilities\.js\?v=1\.59\.139'\)/, 'optional backend capability checks must stay outside the startup graph');
...
assert.match(source, /theaterCatalog:[\s\S]*Promise\.all\([\s\S]*loadLocalChunk\('\.\/builtin-theaters\.js\?v=1\.59\.139'\)[\s\S]*loadLocalChunk\('\.\/qianmu-theaters\.js\?v=1\.59\.139'\)/, 'both managed theater catalogs must share one recoverable on-demand feature boundary');
