...
assert.match(iconSource, /if \(icon\.classList\.contains\(ICON_CLASS\)\) icon\.classList\.remove\(ICON_CLASS\)/);
assert.match(iconSource, /if \(icon\.hasAttribute\(ICON_DATA\)\) icon\.removeAttribute\(ICON_DATA\)/);
assert.match(iconSource, /!root\.matches\(ICON_HOST_SELECTOR\) && !root\.querySelector\(ICON_HOST_SELECTOR\)/);
assert.match(iconSource, /applyExistingQianmuIcons\(doc\)/);
assert.match(iconSource, /touchesOwnedRoot/);
