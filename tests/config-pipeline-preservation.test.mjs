...
test('both actual terminal pipeline writers require preservation and keep inline originals on collision',async()=>{
  for(const single of [false,true])for(const conflict of [false,true]) {
    ...
    let saved=0,seen;
    const c=vm.createContext({storyboardState:()=>state,storyboardPipelineArchiveEpoch:0,storyboardPipelineArchiveCache:new Map(),storyboardPipelineArchiveWrites:new Map(),
