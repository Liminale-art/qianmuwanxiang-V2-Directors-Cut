...
test('repeated binding and clicks coalesce; completion only updates live footer fields', async () => {
  ...
  f.resolve({status: 'ready', services: ['doubao-tts'], version: '1.59.373', checkedAt: Date.now()}); await pending;
  assert.equal(f.label.textContent, '正常'); assert.equal(f.label.title, '豆包语音网关');
  assert.equal(f.versions.current.textContent, 'v1.59.373'); assert.equal(f.versions.latest.textContent, '未获取');
...
test('health refresh preserves a separately verified latest version through missing and failed probes', async () => {
  const f=fixture();f.context.optionalServiceState.latestVersion='1.59.373';
  ...
  assert.equal(f.versions.latest.textContent,'v1.59.373');assert.equal(f.versions.current.textContent,'未安装');
  pending=f.context.refreshOptionalServiceState(true);await flush();f.reject(Error('synthetic'));await pending;
  assert.equal(f.versions.latest.textContent,'v1.59.373');assert.equal(f.versions.current.textContent,'未获取');
});
