...
const fixture = () => {
  ...
  const context = vm.createContext({htmlEscape, VERSION:'1.59.384', MODAL_ID: 'fixture', document: {getElementById: () => modal},
...
test('repeated binding and clicks coalesce both service and release checks; completion only updates live footer fields', async () => {
  ...
  f.resolve({status: 'ready', services: ['doubao-tts'], version: '1.59.384', checkedAt: Date.now()}); await pending;
  assert.equal(f.label.textContent, '正常'); assert.equal(f.label.title, '豆包语音网关');
  assert.equal(f.versions.current.textContent, 'v1.59.384'); assert.equal(f.latestLabel.textContent, '配套');assert.equal(f.versions.latest.textContent, 'v1.59.384');
  ...
  assert.equal(f.latestLabel.textContent,'配套');assert.equal(f.versions.latest.textContent,'v1.59.384');
});
...
test('unknown and mismatched backend versions show the paired branch without calling it the installed latest', () => {
  ...
  assert.match(html,/sd-storage-service-latest-label">配套<\/span> <b class="sd-storage-service-latest">v1\.59\.384/);
  assert.doesNotMatch(html,/sd-storage-service-latest-label">最新/);
  f.context.paintOptionalServiceState();assert.equal(f.latestLabel.textContent,'配套');assert.equal(f.versions.latest.textContent,'v1.59.384');
  ...
  assert.match(html,/sd-storage-service-latest-label">配套<\/span> <b class="sd-storage-service-latest">v1\.59\.384/);
  f.context.optionalServiceState={status:'ready',services:[],version:'1.55.0',latestVersion:'1.59.384'};
  html=f.context.renderStorageServiceStatus();assert.match(html,/sd-storage-service-latest-label">配套<\/span> <b class="sd-storage-service-latest">v1\.59\.384/);
  f.context.paintOptionalServiceState();assert.equal(f.latestLabel.textContent,'配套');assert.equal(f.versions.latest.textContent,'v1.59.384');
  f.context.optionalServiceState.version='1.59.384';f.context.paintOptionalServiceState();
  ...
  assert.equal(f.latestLabel.textContent,'配套');assert.equal(f.versions.latest.textContent,'v1.59.384','a failed release check cannot retain a stale latest label');
});
...
test('current and latest versions stay distinct and only verified version strings are displayed', () => {
  ...
  for (const latestVersion of [undefined,null,{},371,'','latest','<img src=x>','1.2.3\n','1.2.3-'+ 'a'.repeat(80)]) {
    ...
    assert.match(f.context.renderStorageServiceStatus(),/sd-storage-service-latest-label">配套<\/span> <b class="sd-storage-service-latest">v1\.59\.384/);
...
test('health refresh preserves a separately verified latest version through missing and failed probes', async () => {
  const f=fixture();f.context.optionalServiceState.latestVersion='1.59.384';
  ...
  assert.equal(f.versions.latest.textContent,'v1.59.384');assert.equal(f.versions.current.textContent,'未安装');
  pending=f.context.refreshOptionalServiceState(true);await flush();f.reject(Error('synthetic'));await pending;
  assert.equal(f.versions.latest.textContent,'v1.59.384');assert.equal(f.versions.current.textContent,'未获取');
});
