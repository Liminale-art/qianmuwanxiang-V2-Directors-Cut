...
assert.equal(manifest.version, '1.56.2');
...
assert.match(source, /chat: \{ images: records, collections: clone\(storyboardGalleryCollections\(\)\)/, '分镜数据打包必须包含合集且不再导出角色绑定');
assert.match(source, /store\.storyboardCollections = storyboardMergeById/, '跨端导入必须恢复合集');

for (const label of ['来源楼层', '正面提示词', '负面提示词', '画师串', '模型与参数', '种子']) {
  assert.match(source, new RegExp(`<dt>${label}<\\/dt>`), `图片详情缺少字段：${label}`);
}
assert.doesNotMatch(source, /<dt>人物档案<\/dt>|<dt>实际最终提示词<\/dt>/, '阅片详情不得恢复已切割的角色链路或重复最终提示词');
