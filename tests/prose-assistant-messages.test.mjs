...
test('formal prompts are mandatory and malformed, future, duplicate or over-budget data is rejected without truncation',()=>{
...
test('combined pipeline keeps incomplete raw text visible but out of later history and never sends without configured prompt',async()=>{
  ...
  await assert.rejects(f.session.run({question:'Q2',source:f.source,request:unconfigured.send}));assert.equal(f.sent.length,0);assert.equal(f.listeners(),0);f.session.close();
});
