...
assert.match(await readFile(new URL('../qianmu-reader-package.js', import.meta.url), 'utf8'), /comicDescriptions: rec\?\.comicDescriptions/, '独立数据收集模块必须携带漫画视觉文字稿');
assert.match(source, /await collectCoreadPackageData[\s\S]*version: 5/, '数据打包仍调用资料收集模块并沿用v5格式');
