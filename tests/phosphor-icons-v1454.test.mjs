...
assert.equal(currentFaNames.length, 132, '入口当前发出的 Font Awesome 图标名基线必须保持为 132 个');
assert.equal(QIANMU_CURRENT_FA_ICON_COUNT, 132);
...
assert.match(indexSource, /from '\.\/qianmu-icon-renderer\.js\?v=1\.49\.0';/, '图标渲染器 import 必须按发布版本破缓存');
...
console.log('v1.49.0 Phosphor icon boundary and coverage contract OK');
