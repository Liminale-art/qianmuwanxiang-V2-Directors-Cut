...
const functions=['focusClockSound','focusClockResetMedia','focusClockPrimeSound','focusClockPlayDoneSound'];
...
function fixture(){
  ...
  const c=vm.createContext({Audio,createFocusSoundPlayer,focusClockSoundPlayer:null,
...
test('preview keeps one audio element and one frame, supports pause/resume without losing seek position',async()=>{
  ...
  audio.ended=true;audio.dispatch('ended');assert.equal(frames.size,0);assert.equal(audio.currentTime,0);assert.equal(c.focusClockSound().snapshot().previewMode,false);
});
...
test('reset invalidates pending warmup fulfillment and rejection without reviving or touching released media',async()=>{
  for(const rejected of [false,true]){
    ...
    assert.equal(c.focusClockSound().snapshot().hasMedia,false);assert.equal(frames.size,0);assert.equal(c.focusClockSound().snapshot().previewMode,false);
  }
});
...
test('late ended from an old source cannot reset a newer preview or cancel its frame',async()=>{
  ...
  audios[0].dispatch('ended');assert.equal(c.focusClockSound().snapshot().previewMode,true);assert.equal(frames.size,1);
});
