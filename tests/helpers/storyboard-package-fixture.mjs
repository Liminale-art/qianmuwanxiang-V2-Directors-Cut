...
export function createPackageImportFixture(){
  ...
  const journal={assertNoHistoricalChatMutation:async()=>{if(e.historical)throw Error('本账户有原聊天恢复记录，请在数据管理查看或明确结束');},loadResource:async()=>e.resource||null,dismissResource:async row=>{assert.deepEqual(row,e.resource);e.resource=null;},loadMutation:async()=>e.pending&&structuredClone(e.pending),prepareMutation:async row=>{e.events.push('journal');if(e.pending)throw Error('pending');e.pending=structuredClone(row);return structuredClone(row);},
