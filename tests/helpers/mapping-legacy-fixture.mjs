...
export function mappingLegacyFixture(rows=[],{indexed=true}={}){
  const state={rows:structuredClone(rows),reads:[],indexed,error:false};
  const indexedDB={open(){const request={};queueMicrotask(()=>{
    ...
    request.result={close(){},transaction(names,mode){
      ...
      const values=name=>name==='mappingHeads'?(state.indexed?state.rows.map(row=>mappingHead(row.kind,row.receipt)):[]):state.rows.filter(row=>row.kind===(name==='environmentMaps'?'environment':'subjects')).map(row=>row.receipt);
      tx.objectStore=name=>({get:key=>ask(()=>values(name).find(row=>row.key===key)),index:()=>({
