...
export function mappingLegacyFixture(rows=[],{indexed=true}={}){
  ...
  const indexedDB={open(){const request={};queueMicrotask(()=>{
    ...
    request.result={close(){},transaction(names,mode){
      ...
      const values=name=>name==='mappingHeads'?(state.indexed?state.rows.map(row=>mappingHead(row.kind,row.receipt)):[]):['environmentMaps','subjectMaps'].includes(name)?state.rows.filter(row=>row.kind===(name==='environmentMaps'?'environment':'subjects')).map(row=>row.receipt):[];
