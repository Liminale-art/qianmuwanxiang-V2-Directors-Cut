...
export async function assistantManagerFixture(t,count=2,patch={}){
 ...
 const f={transport,store,live:true,owner:namespace,records:[],writes:0,closes:0,loads:0,legacy:new Map()};
 ...
 f.options={resolveNamespace:async()=>f.owner,isCurrent:()=>f.live,loadPage:input=>f.loadPage(input),now:()=>200,legacyFactory:()=>({read:async(a,key,{guard})=>{if(guard()!==true)throw Error('stale');return structuredClone(f.legacy.get(key)||emptyProseAssistantHistory(key));},close(){}}),
  storageFactory:async options=>{const native=await transport.createStorage(options);return {...native,close(){f.closes++;native.close();},async write(...args){f.writes++;if(f.beforeWrite)await f.beforeWrite(...args);const result=await native.write(...args);if(f.afterWrite)await f.afterWrite(...args);return result;}};},...patch};
