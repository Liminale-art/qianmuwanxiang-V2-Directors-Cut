...
test('duplicate current commit or current unit declarations are rejected instead of choosing an arbitrary match',()=>{
  const f=fixture();f.documents[names[1]]=f.documents[names[1]].replace('## Historical',`- 当前提交：\`${head}\` / v${version}\n\n## Historical`);assert.throws(()=>check(f),{code:'current_commit_ambiguous'});
  const g=fixture();g.documents[names[1]]=g.documents[names[1]].replace('## Historical','- 当前单元：R2-04 second\n\n## Historical');assert.throws(()=>check(g),{code:'current_unit_ambiguous'});
});
