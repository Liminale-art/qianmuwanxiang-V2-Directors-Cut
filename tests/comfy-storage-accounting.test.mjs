...
import {renderStorageBackupSection,storageOverviewSegments} from '../qianmu-storage-backup-view.js';
...
test('actual storage card shows compact Comfy totals and unknown status without duplicate library links',()=>{
  ...
  const context=vm.createContext({renderStorageBackupSection,storageOverviewSegments,VERSION:'1.59.391',optionalServiceState:{status:'idle',services:[]},storageInventoryState:{status:'ready',data:{sampledAt:1,origin:{available:true,usage:99999,quota:999999},trackedBytes:comfy.bytes,categories:[],idb:{stores:[]},comfyStorage:comfy}},STORAGE_CATEGORY_LABELS:{},STORAGE_CATEGORY_COLORS:{},htmlEscape:x=>String(x??'').replaceAll('<','&lt;'),formatStorageBytes:x=>`${x} B`});
  context.storageInventoryState.data.origin.pressure={};vm.runInContext(['optionalServiceLabel','optionalServiceLatestDisplay','optionalServiceDetail','renderStorageServiceStatus','renderStorageManagementCard'].map(section).join('\n'),context);const html=context.renderStorageManagementCard();
  assert.match(html,/<span>工作流<\/span><span>暂未读取<\/span>/);assert.doesNotMatch(html,/<span>工作流<\/span><span>0 B|data-storage-comfy-library|含 2 个版本|unreadable <data>/);assert.match(html,new RegExp(`<span>工作流方案</span><span>${comfy.pools.bytes} B</span>`));assert.match(html,/浏览器暂存空间/);assert.doesNotMatch(html,/未盘点站点数据/);assert.match(html,/部分数据暂不可读取/);
});
