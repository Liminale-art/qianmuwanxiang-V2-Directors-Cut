...
assert.match(source, /function importPinnedNotesBackup[\s\S]*importQianmuNotesBackup\(file, \{check, read:\(\)=>listQianmuNotes\(\{strict:true\}\), write:note=>saveImportedQianmuNote\(note,\{check\}\), uid\}\)/, 'the guarded entry must use strict inventory and create-only committed import writes');
