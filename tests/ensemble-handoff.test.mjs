...
import {attachEnsembleCompilerResult,sealEnsembleCompilerResult,resolveEnsembleCompiledRoutes} from '../qianmu-ensemble-handoff.js?v=1.59.261';
...
function fixture(){
  ...
  const receipt={executionAuthorized:false,assignments:result.shots.map((_,i)=>({shotId:'S'+(i+1)}))};
  const session={assertCurrent(){if(!live)throw Error('expired');},async resolveAssignment(value,id){this.assertCurrent();assert.equal(value,receipt);calls.push(id);if(hook)await hook();this.assertCurrent();return {shotId:id,schemeId:id,bindingKey:id,route:{providerId:'novel',modelId:id},artistPresetId:'artist-'+id};}};
