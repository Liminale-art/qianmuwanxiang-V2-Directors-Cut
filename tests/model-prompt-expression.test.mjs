...
test('real prose compiler creates one multi-format plan and mixed NAI/Comfy generation consumes each correct expression in narrative order',async()=>{
  ...
  assert.equal(await e.context.storyboardGenerate(null),true,JSON.stringify(e.notices));await e.awaitScheduled();assert.equal(e.llmCalls.length,2);assert.equal(e.jobs.length,3);
