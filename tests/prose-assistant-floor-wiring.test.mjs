...
test('assistant uses the existing host refresh and cleanup, outside storyboard enablement, without loading its panel eagerly',async()=>{
  ...
  assert.doesNotMatch(floor,/^import.*panel|new MutationObserver|setInterval\(/m);assert.match(floor,/await loadLocalChunk\('\.\/qianmu-prose-assistant-panel.js'\)/);assert.match(floor,/floorCollectionText\(candidate\?\.querySelector\('\.mes_text'\)\)/);
