...
import {isRichProse} from '../qianmu-prose-rich-compat.js';
...
function fixture({width=160,viewport=360,avatar=false}={}){
  ...
  const context=vm.createContext({document:{getElementById:()=>chat},window:{innerWidth:viewport},getComputedStyle:()=>({paddingLeft:'20px',paddingRight:'40px'}),proseLayoutSettings:()=>layout,proseLayoutMessageRoots:()=>[text],isRichProse});
...
test('a regex-styled card never inherits expanded prose width, even when added after layout',()=>{
  const e=fixture();e.run();assert.equal(e.text.dataset.sdProseExpanded,'1');
  e.context.isRichProse=()=>true;e.run();
