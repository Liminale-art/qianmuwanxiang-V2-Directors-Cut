...
function fixture({respond}={}){
  ...
  const client=createComfyRecoveryClient({origin:'https://st.test',account:async()=>account,
    ...
    fetchImpl:async(url,init)=>{const body=JSON.parse(init.body);calls.push({url,body});return respond?respond(url,body):json(url.includes('/cloud/')?cloud:native);}});
...
test('one catalog merges local and cloud originals and carries explicit immutable cloud identity',async()=>{
  ...
  assert.ok(f.calls.every(call=>call.url.endsWith('/catalog')&&!call.body.apiKey));
});
