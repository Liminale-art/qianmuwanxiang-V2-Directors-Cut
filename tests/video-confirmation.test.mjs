...
test('a ready draft still requires one explicit price acknowledgement', () => {
  ...
  assert.deepEqual(review.confirmationIssues, ['cost_confirmation_required']);
...
test('2K requires a separate acknowledgement from the price acknowledgement', () => {
  ...
  const second = createVideoGenerationConfirmation(input({
    spec: { ...input().spec, resolution: '2k' },
    acknowledgements: { cost: true, highResolution: true },
  }), { now: NOW });
...
test('the confirmation fingerprint invalidates any changed prompt or generation setting', () => {
  const base = createVideoGenerationConfirmation(input({ acknowledgements: { cost: true } }), { now: NOW });
  const changedPrompt = createVideoGenerationConfirmation(input({ prompt: prompt.replace('quiet kitchen', 'rainy kitchen'), acknowledgements: { cost: true } }), { now: NOW });
  const changedDuration = createVideoGenerationConfirmation(input({ spec: { ...input().spec, durationSeconds: 8 }, acknowledgements: { cost: true } }), { now: NOW });
...
test('the confirmation contract stays lazy and only submits after the explicit fingerprinted gate', async () => {
  ...
  assert.match(source, /videoConfirmation:\s*\{[\s\S]*qianmu-video-confirmation\.js\?v=1\.58\.78/);
