...
assert.match(indexSource, /const compilerConfig=storyboardCompilerRequestConfig\(stream\?\{\.\.\.state,pendingParagraphSelection:null\}:state,profile,expressionRoutes\);[\s\S]*?buildStoryboardPlanContractRequest\(context,compilerConfig\)/, 'the first planning call must use the strict request contract, ignore manual selections only in isolated automatic handoffs, and retain verified mixed model formats');
assert.match(indexSource, /if\(compilerConfig\.styleSession\?\.enabled&&compilerConfig\.styleSession\.styleLock&&!compilerConfig\.manualSupplement\)await contract\.captureStoryboardEnsembleHistory\(context\.compilerSources,\[\.\.\.state\.logs,\.\.\.storyboardGalleryRecords\(\)\]\);[\s\S]*?buildStoryboardPlanContractRequest\(context,compilerConfig\)/, 'historical scene anchors are verified only for an enabled style lock, before constructing the first request');
assert.match(indexSource, /inputGuard\.compilerAttempt\.call\(contractRequest\.messages[\s\S]*jsonSchema: contractRequest\.schema[\s\S]*jsonSchemaStrict: true/, 'capable external channels receive the structured schema through request-local diagnostics');
assert.match(await readFile(new URL('../qianmu-storyboard-compiler-result.js',import.meta.url),'utf8'), /if \(contractRequest \|\| declaresPlanContract\)/, 'new requests must validate strictly while versioned legacy responses remain supported');
