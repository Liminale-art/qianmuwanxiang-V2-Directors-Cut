...
assert.match(indexSource, /buildStoryboardPlanContractRequest\(context, storyboardCompilerRequestConfig\(stream\?\{\.\.\.state,pendingParagraphSelection:null\}:state, profile, expressionRoutes\)\)/, 'the first planning call must use the strict request contract, ignore manual selections only in isolated automatic handoffs, and retain verified mixed model formats');
