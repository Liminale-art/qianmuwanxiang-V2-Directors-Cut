...
assert.match(source, /routedConnection[\s\S]*(?:const|let) providerProfile = storyboardResolveRoutingProfile[\s\S]*capabilityModelId:[\s\S]*storyboardGenerationPayload\(state, providerProfile/, '镜组任务必须使用同一绑定解析链生成有效参数');
...
assert.match(source, /pruneStoryboardRetakeGallery\(gallery,records,\(storyboardState\(\)\.shotPlans\|\|\[\]\)\.map\(plan=>plan\.floorTake\),takeReceipts\)/, '成片索引修剪必须保留本轮回执、尚未收片重拍的旧作和持久换版保护');
