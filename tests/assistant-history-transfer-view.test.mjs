...
test('storage offers management/import even with no data or backend and real entry passes the cleanup lifetime guard',async()=>{
 const html=renderStorageBackupSection({htmlEscape:value=>String(value)});assert.match(html,/场外特助会话/);assert.match(html,/sd-storage-assistant-library/);assert.match(html,/备份 \/ 恢复/);
