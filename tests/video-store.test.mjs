...
test('storage UI identifies video records as destructive and the adapter stays idle at startup', async () => {
  ...
  assert.match(source, /videoStore:\s*\{[\s\S]*import\('\.\/qianmu-video-store\.js\?v=1\.58\.56'\)/);
  assert.match(source, /videoResult:\s*\{[\s\S]*import\('\.\/qianmu-video-result\.js\?v=1\.58\.56'\)/);
