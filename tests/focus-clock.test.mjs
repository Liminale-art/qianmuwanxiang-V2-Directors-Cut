...
const scenePrompt = await readFile(new URL('../qianmu-focus-scene-prompts.js', import.meta.url), 'utf8');
...
const preparation = await readFile(new URL('../qianmu-focus-preparation.js', import.meta.url), 'utf8');
...
assert.match(session, /function start[\s\S]*f\.endsAt = now \+ remaining/, '开始和继续必须写入真实截止时间');
assert.match(session, /function pause[\s\S]*f\.remainingMs = Math\.max\(0, f\.endsAt - now\)[\s\S]*f\.status = 'paused'/, '暂停必须把截止时间折算为剩余时长');
...
assert.match(events, /if \(nextUrl !== f\.soundUrl\)[\s\S]*sound\.reset/, '外链未变化时不得在每次点击试听时重建音频，否则暂停无法生效');
...
assert.match(scenePrompt, /你是“千幕专注场景”的角色短句编写器[\s\S]*不引用聊天正文[\s\S]*不得猜测正文情节/, '情景生成提示词必须与正文隔离并约束不 OOC');
...
assert.match(cueRecords, /function fileBase[\s\S]*speaker[\s\S]*task[\s\S]*formatStamp/, '专注角色语音命名必须包含角色、任务与时间');
