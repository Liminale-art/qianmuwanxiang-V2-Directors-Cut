...
const response=(version='1.59.382',extra={})=>new Response(JSON.stringify({name:'qianmu-omniscene',version,...extra}));
...
test('release lookup uses the ST-verified official branch without credentials, redirects or request headers',async()=>{
  ...
  assert.equal(await read(official),'1.59.382');assert.equal(requests.length,1);
...
test('same-branch checks are single-flight and cached for thirty minutes including failures',async()=>{
  ...
  const a=read(official),b=read(official);assert.equal(calls,1);finish(response());assert.deepEqual(await Promise.all([a,b]),['1.59.382','1.59.382']);
  now=30*60*1000-1;assert.equal(await read(official),'1.59.382');assert.equal(calls,1);
  ...
  const d=read({...official,currentBranchName:'codex/next'});assert.equal(calls,3);finish(response('1.59.382'));assert.equal(await d,'1.59.382');
...
test('package identity and version are validated without borrowing the installed frontend version',async()=>{
  for(const body of [{name:'different-package',version:'1.59.382'},...['latest','<script>','1.2.3\n','1.2.3-'+ 'a'.repeat(80),{},null].map(version=>({name:'qianmu-omniscene',version}))]){
  ...
  assert.equal(await createQianmuReleaseVersionReader({fetchImpl:async()=>response('v1.59.382-rc.1+build.2')})(official),'1.59.382-rc.1+build.2');
...
function updateFixture(){
  ...
  const context=vm.createContext({AbortController,setTimeout,clearTimeout,Date,
    qianmuUpdateState:{status:'idle',checkedAt:0},qianmuUpdatePromise:null,optionalServiceState:{status:'ready',version:'1.59.382',services:[]},
...
test('the existing update check starts release metadata asynchronously and does not hold health or repeat on cached opens',async()=>{
  ...
  assert.equal(f.context.optionalServiceState.status,'ready');assert.equal(f.context.optionalServiceState.version,'1.59.382');
  f.pending[0].resolve('1.59.382');await flush();assert.equal(f.context.optionalServiceState.latestVersion,'1.59.382');
...
test('superseded release responses cannot overwrite a newer check and failed ST checks leave latest unknown',async()=>{
  ...
  f.pending[1].resolve('1.59.382');await flush();f.pending[0].resolve('1.59.371');await flush();assert.equal(f.context.optionalServiceState.latestVersion,'1.59.382');
