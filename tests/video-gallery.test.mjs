...
test('the dynamic gallery ships as an idle feature chunk', async () => {
  ...
  assert.match(source, /videoGallery:\s*\{[\s\S]*import\('\.\/qianmu-video-gallery\.js\?v=1\.58\.72'\)/);
  assert.match(source, /videoStore:\s*\{[\s\S]*import\('\.\/qianmu-video-store\.js\?v=1\.58\.72'\)/);
