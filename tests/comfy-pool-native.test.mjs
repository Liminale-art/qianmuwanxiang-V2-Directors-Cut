...
test('native recovery UI uses one compact snapshot, shows retained source choices and never changes the current selection',async()=>{
  ...
  try{c.mount(host);await flush();assert.equal(views,1);assert.match(host.innerHTML,/ST 账户保存/);assert.doesNotMatch(host.innerHTML,/<old>/);buttons['copy-legacy'].click();await flush();assert.deepEqual(calls,[{census,id:'old',choice:'copy',expectedRevision:7,confirmed:true}]);buttons['export-legacy'].click();await flush();assert.equal(downloads.length,1);assert.equal(selected,0);}finally{c.dispose();}
