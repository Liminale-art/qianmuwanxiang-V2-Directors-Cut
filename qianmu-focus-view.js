  ...
  const voiceConfig = `
    <div class="sd-focus-voice-config">
      <div class="sd-focus-voice-grid">
        <div class="sd-focus-voice-field"><span>${f.activity === 'reading' ? '书友' : '角色'}</span>${f.activity === 'reading' ? `<input class="text_pole" aria-label="书友" value="${htmlEscape(voiceContext.characterName)}" readonly>` : `<select class="text_pole sd-focus-voice-character" aria-label="角色" ${locked ? 'disabled' : ''}>${voiceCharacterOptions}</select>`}</div>
      ...
      ${f.voiceMode==='custom'?'<button type="button" class="sd-btn sd-focus-library-open"><i class="fa-solid fa-folder-open"></i>台词库</button>':''}
