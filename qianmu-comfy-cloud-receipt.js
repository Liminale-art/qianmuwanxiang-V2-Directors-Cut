...
export function normalizeComfyCloudReceipt(value) {
  try {
    ...
    const stillOutput = normalizeComfyReceipt(value.stillOutput);
    return freeze({ schema: COMFY_CLOUD_RECEIPT_SCHEMA, task, requestDigest: value.requestDigest,
      workflow: { templateHash, executionHash, ...(binding ? { binding } : {}) }, stillOutput });
