...
export function createEnsembleSceneLock(narrative,{enabled=false,assertCurrent}={}){
  ...
  return Object.freeze({constraints,assertCurrent:current,validate(assignments){
    ...
    for(const group of constraints.groups){const scheme=chosen.get(group.leader_shot_id);if(group.shot_ids.some(id=>chosen.get(id)!==scheme))fail();}
    current();return true;
  }});
}
