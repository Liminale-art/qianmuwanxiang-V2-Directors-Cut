...
export function createProseAssistantFloorTools({getContext,resolveNamespace,headers,applyIcons,mountPortal,notify,isCurrent,assistantConfig,confirm,download,assistantSettings,saveAssistantSettings,assistantHistoryFactory}={}){
  ...
  async function storageSummary(valid){
    ...
    try{module=await import('./qianmu-prose-assistant-storage.js?v=1.59.367');}catch{if(!live())throw Error('助手储存页面已变化');return {status:'unavailable',bytes:null,count:null,error:'助手统计组件未加载，请刷新重试。'};}
  ...
  async function cleanupStorage(parent,confirm,check,expectedNamespace,otherModules=0,native=false){
    ...
    try{
      if(native){
        const runtime=await import('./qianmu-assistant-history-view.js?v=1.59.367');check();
        if(!valid()||await resolveNamespace()!==expectedNamespace)throw Error('助手管理账户已变化');check();
        ...
        try{opened=runtime.openAssistantHistoryManager({parent:portal,resolveNamespace:async()=>{const actual=await resolveNamespace();if(actual!==expectedNamespace)throw Error('助手管理账户已变化');return actual;},isCurrent:valid,headers,check,confirm,download,otherModules,applyIcons});management=opened;await opened.finished;}
        finally{opened?.dispose();if(management===opened)management=null;detach?.();portal.remove();}return;
      }
      const runtime=await import('./qianmu-prose-assistant-storage.js?v=1.59.367');check();
