...
export function summarizeGalleryRecords(records, collectionIds) {
  const byId = new Map(), counts = new Map(), rawTags = new Set(), known = new Set(), words = new Set(), legacyTags = [];
  for (const record of records) {
    if (!byId.has(record.id)) byId.set(record.id, record);
    for (const id of new Set(collectionIds(record))) counts.set(id, (counts.get(id) || 0) + 1);
