...
export function renderUserAliasReview({preview,busy,accepted,notice}){
  ...
  return `<header><b>核对 USER 地址</b><button type="button" class="sd-icon-btn" data-alias-action="close" aria-label="关闭"><i data-qm-icon="qm-regular-x"></i></button></header><main>
    ...
    <p>${preview?(total?`${total} 条原绑定 · ${preview.groups} 组 · 待选择 ${preview.unresolved} 组`:'未发现需要整理的等价头像地址'):busy?'正在核对本机绑定…':'本机绑定核对未完成，请重新核对。'}${preview?.missing?` · ${preview.missing} 个人设不在ST目录中，请先核对原头像文件`:''}</p>
    <fieldset ${busy?'disabled':''}>${(preview?.rows||[]).map((row,index)=>`<label class="sd-user-alias-item"><input type="radio" name="${row.groupId}" data-alias-choice="${index}" data-alias-candidate="${row.candidateId}" ${row.selected?'checked':''} ${row.present?'':'disabled'}><span><b>${escape(row.archiveName||row.archiveId||'不使用档案（显式解除）')}</b><small>${row.scope==='default'?'默认绑定':`聊天 · ${escape(row.chatKey)}`}</small><small>${escape(row.sourceKey)}</small><small>统一为 ${escape(row.targetKey)}</small><small>${row.conflict?'同组存在不同档案，请明确选择':row.selected?'此组绑定一致，确认后整理':'此组绑定一致，原关系仍留凭据'}${row.present?'':' · 人设未找到'}</small></span></label>`).join('')}
...
export function openUserAliasReview({parent,run,icons=()=>{}}){
  ...
  const abort=new AbortController(),state={preview:null,choices:{},busy:false,accepted:false,notice:''};let closed=false,resolve,focusField=null;const finished=new Promise(done=>resolve=done);
  const returnFocus=document.activeElement;
  const close=()=>{if(closed)return;closed=true;abort.abort();if(dialog.open)dialog.close();dialog.remove();if(returnFocus?.isConnected)returnFocus.focus({preventScroll:true});resolve();};
  const active=()=>{if(closed)return false;if(!dialog.isConnected){close();return false;}return true;};
  const draw=(scroll=0)=>{
    if(!active())return;const element=dialog.contains(document.activeElement)?document.activeElement:null;
    const field=element?.hasAttribute('data-alias-candidate')?element.getAttributeNode('data-alias-candidate'):element&&[...element.attributes].find(attribute=>attribute.name.startsWith('data-alias-'));
    if(field)focusField={name:field.name,value:field.value};
    dialog.innerHTML=renderUserAliasReview(state);icons(dialog);dialog.querySelector('main').scrollTop=scroll;
    if(!state.busy&&focusField){const next=[...dialog.querySelectorAll('input,button')].find(node=>node.getAttribute(focusField.name)===focusField.value&&!node.matches(':disabled'))||dialog.querySelector('[data-alias-action="refresh"]');next?.focus({preventScroll:true});}
  };
  const request=async(action,input)=>{if(!active())throw Error('USER地址核对页面已关闭');const result=await run(action,{input,signal:abort.signal});if(!active())throw Error('USER地址核对页面已关闭');return result;};
  async function work(action,offset=0){
    if(!active()||state.busy||action==='apply'&&(!state.preview?.ready||!state.accepted))return;
    ...
    try{
      if(action==='apply'){
        const result=await request('user-alias-apply',{choices:state.choices,digest:approved.digest,confirmed:true});
        state.preview=null;state.choices={};state.notice=`已核对整理 ${result.before} 条原绑定，保留 ${result.after} 条有效绑定。原关系可在储存空间→迁移映射凭据中查看与导出。`;
      }
      state.preview=await request('user-alias-preview',{choices:state.choices,offset});
    }catch(error){if(!active())return;state.preview=null;state.choices={};state.notice=[state.notice,error?.message||'整理结果未确认，请重新核对'].filter(Boolean).join(' ');}
  ...
  dialog.addEventListener('click',event=>{
    ...
    if(action==='refresh')void work('preview');else if(action==='apply')void work('apply');
