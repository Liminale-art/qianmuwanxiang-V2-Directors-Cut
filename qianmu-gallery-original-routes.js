...
import {galleryOriginalError,galleryOriginalErrorPayload,galleryOriginalCapabilities,galleryOriginalPreserved,galleryOriginalBatchPreserved,GALLERY_ORIGINAL_MAX_BYTES} from './qianmu-gallery-original-contract.js';
...
export function installGalleryOriginalRoutes(router,{dataRoot,register,serviceOptions={}}){
    let service;
    for(const action of ['capabilities','preserve','preserve-batch','read'])router[action==='capabilities'?'get':'post']('/chat-gallery/original/'+action,async(req,res)=>{
        ...
        try{
            ...
            const result=await service[action==='preserve-batch'?'preserveBatch':action](req,req.body,{signal:controller.signal});
            ...
            if(action==='preserve-batch')return res.json(galleryOriginalBatchPreserved(result));
