...
const names={configuration:'配置恢复副本',vibes:'Vibe 素材暂存记录',characters:'角色恢复记录',bundle:'联包恢复记录','historical-chat':'原聊天恢复记录'};
...
const historicalPhases={prepared:'待保存聊天资料',submitted:'聊天保存已发起，待核对',uncertain:'聊天资料保存未确认',verified:'聊天资料已核对；原件仍需检查'};
export function renderRestoreStorageReview({summary,busy,notice,selected,accepted,chatHash},formatBytes){
  return `<header><b>分镜恢复记录</b><button type="button" class="sd-icon-btn" data-restore-storage="close" title="关闭" aria-label="关闭"><i data-qm-icon="qm-regular-x"></i></button></header>
    ...
    ${summary?.items.some(row=>row.kind==='vibes')?'<p>素材暂存只记录上次核对进度，不证明原件仍然存在。继续时须重新核对完整原包与素材，不自动应用配置或生成；结束待办不表示释放服务器磁盘。</p>':''}
    ${summary?.items.some(row=>row.kind==='historical-chat')?'<p>原聊天恢复记录通过当前 ST 账户保存，旧本机记录仍保留。记录包含画面、相册与人物草稿的修改前后内容，跨端继续前仍须核对原包与准确聊天。结束后不再将本记录用作对应引用的续接依据；仅标记已结束并保留副本，不会删除服务器聊天或原图，也不证明整包恢复完成。</p>':''}
    <fieldset ${busy?'disabled':''}>${(summary?.items||[]).map((row,index)=>`<label class="sd-restore-storage-item"><input type="checkbox" data-restore-item="${index}" ${selected.has(index)?'checked':''}><span><b>${names[row.kind]}</b><small>${escape(row.kind==='historical-chat'?historicalPhases[row.phase]:phases[row.phase])} · ${escape(formatBytes(row.bytes))}</small><small>${row.kind==='historical-chat'?`原聊天定位 · ${escape(row.targetHash.slice(0,10))}`:row.chatHash?(row.chatHash===chatHash?'当前聊天':`其他聊天 · ${escape(row.chatHash.slice(0,10))}`):'账户级角色库'} · 原包 ${escape(row.fileHash.slice(0,10))}</small><small>${escape(new Date(row.updatedAt).toLocaleString())}</small></span></label>`).join('')}
