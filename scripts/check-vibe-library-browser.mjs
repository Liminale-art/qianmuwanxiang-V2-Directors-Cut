...
try{
  ...
  previewBytes=Buffer.from(await page.evaluate(async()=>{
    ...
    window.startVibes=({delayPreview=false,selection=true}={})=>{
      ...
      controller.edit(rows[0]);if(selection)controller.beginSelection({id:'synthetic-selection',ids:['vibe-1']});controller.mount(host);
    };
  ...
  for(const [family,mode] of [['classic','dark'],['editorial','light'],['editorial','dark'],['glass','light'],['glass','dark']]){
    ...
    for(const width of [320,393,1100]){
      ...
      await page.locator('.sd-storyboard-create-vibe').click();await page.waitForFunction(()=>calls.saves===1&&document.querySelector('.sd-storyboard-vibe-name').value==='');
