...
try{
  ...
  for(const width of [320,393,1280])for(const skin of ['classic','editorial','glass'])for(const mode of ['light','dark']){
    ...
    await page.evaluate(({skin,mode})=>{
      ...
      const environment={qianmuVersion:'1.59.387',stVersion:'1.12.0',backendStatus:'ready'};
    ...
    assert.match(exported.saved,/千幕版本：1.59.387/);assert.match(exported.saved,/ST 版本：1.12.0/);assert.doesNotMatch(exported.saved,/apiKey|workflow|https:|private read/);
