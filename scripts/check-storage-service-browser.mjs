...
try{
  ...
  await page.evaluate(async source=>{
    ...
    Object.assign(window,{MODAL_ID:'story-director-modal',VERSION:'1.59.390',STORAGE_CLEANUP_LAYER_ID:'qianmu-storage-cleanup-layer',THEME_KEYS:['light','dark'],NOTES_THEME_VARIABLES:[],STORAGE_ITEM_RISK:{},STORAGE_CHAT_CLEARABLE:new Set(['audio']),
    ...
    window.setup=async(family,mode,status='ready')=>{
      ...
      window.optionalServiceState={status,services:[],version:'1.59.390',latestVersion:'1.59.390'};
  ...
  for(const width of [320,1280])for(const family of ['classic','editorial','glass'])for(const mode of ['light','dark']){
    ...
    for(const status of ['idle','checking','ready','missing','unsupported','error']){
      ...
      assert.equal(result.current,status==='ready'?'v1.59.390':status==='missing'?'未安装':'未获取');assert.equal(result.latest,'v1.59.390');
  ...
  const live=await page.evaluate(async()=>{
    ...
    window.optionalServiceState={status:'idle',services:[],latestVersion:'1.59.390',checkedAt:0};window.optionalServiceProbePromise=null;
    ...
    const pending=optionalServiceProbePromise;resolveProbe({status:'ready',version:'1.59.390',services:[],checkedAt:Date.now()});await pending;
  ...
  assert.equal(live.calls,1);assert.equal(live.sameCard,true);assert.equal(live.sameInput,true);assert.equal(live.current,'v1.59.390');assert.equal(live.latest,'v1.59.390');checks.push('live refresh coalesces and preserves unsaved input, selection, card, and known latest');
