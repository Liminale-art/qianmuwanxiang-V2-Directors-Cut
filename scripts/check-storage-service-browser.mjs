...
try{
  ...
  await page.evaluate(async source=>{
    ...
    window.setup=async(family,mode,status='ready')=>{
      ...
      window.optionalServiceState={status,services:[],version:'1.59.380',latestVersion:'1.59.380'};
  ...
  for(const width of [320,1280])for(const family of ['classic','editorial','glass'])for(const mode of ['light','dark']){
    ...
    for(const status of ['idle','checking','ready','missing','unsupported','error']){
      ...
      assert.equal(result.current,status==='ready'?'v1.59.380':status==='missing'?'未安装':'未获取');assert.equal(result.latest,'v1.59.380');
  ...
  const live=await page.evaluate(async()=>{
    ...
    window.optionalServiceState={status:'idle',services:[],latestVersion:'1.59.380',checkedAt:0};window.optionalServiceProbePromise=null;
    ...
    const pending=optionalServiceProbePromise;resolveProbe({status:'ready',version:'1.59.380',services:[],checkedAt:Date.now()});await pending;
  ...
  assert.equal(live.calls,1);assert.equal(live.sameCard,true);assert.equal(live.sameInput,true);assert.equal(live.current,'v1.59.380');assert.equal(live.latest,'v1.59.380');checks.push('live refresh coalesces and preserves unsaved input, selection, card, and known latest');
