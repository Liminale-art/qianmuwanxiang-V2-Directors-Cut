...
try{
  ...
  for(const width of [320,393,1280])for(const skin of ['classic','editorial','glass'])for(const mode of ['light','dark']){
    ...
    assert.deepEqual(typing,{same:true,focused:true,draft:'中文方案',reads:before,writes:0},key);
    ...
    const geometry=await page.evaluate(()=>{
      ...
      return {overflow:root.scrollWidth-root.clientWidth,bounds:box(root),fields:[...root.querySelectorAll('.text_pole:not(textarea),button')].map(el=>({tag:el.tagName,...box(el)})),headings:[...root.querySelectorAll('.sd-ensemble-fields label>span')].map(el=>getComputedStyle(el).textAlign),icons:root.querySelectorAll('button svg[stroke-width="2.25"]').length,shadow:getComputedStyle(document.querySelector('[data-ensemble-field=description]')).boxShadow};
    });
    assert.ok(geometry.overflow<=1,key+JSON.stringify(geometry));assert.ok(geometry.icons>=5,key);assert.equal(geometry.shadow,'none',key);
    ...
    await page.locator('[data-ensemble-archived]').check();
    assert.equal(await page.locator('.sd-ensemble-scheme').count(),1,key);
    await page.locator('[data-ensemble-action=archive][data-id=ink]').click();await page.waitForFunction(()=>!mount.model.snapshot().busy);
    assert.equal(await page.locator('.sd-ensemble-scheme').count(),0,key);
