    ...
    for (const [family, mode] of [['classic', 'dark'], ['editorial', 'light'], ['editorial', 'dark'], ['glass', 'light'], ['glass', 'dark']]) {
        for (const width of [320, 393, 1100]) for (const state of ['home', 'read', 'edit', 'favorites', 'error']) {
            ...
            if (family !== 'classic') {
                ...
                if (state === 'home') ok(label + ' original script library follows the family', await page.locator('[data-lib=script] .sd-lib-folder').first().evaluate((node, family) => getComputedStyle(node).borderRadius === (family === 'glass' ? '22px' : '0px'), family));
