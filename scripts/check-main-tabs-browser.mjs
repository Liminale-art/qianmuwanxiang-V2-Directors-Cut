...
try{
 ...
 for(const [family,mode,theme=mode] of families)for(const width of[320,393,1280]){
  ...
  ok(label+' fixed gutters at start, middle and both endpoints',Object.values(sequence).filter(v=>typeof v==='object').every(g=>Math.abs(g.left-16)<=.5&&Math.abs(g.right-16)<=.5));
  ok(label+' revealing a tab never moves document, modal or body',sequence.stable);
  if(sequence.start.max>2){
   ok(label+' first and last endpoints keep equal outer distance',Math.abs(sequence.start.first-sequence.end.last)<=.5&&Math.abs(sequence.start.first-sequence.physicalEnd.last)<=.5&&Math.abs(sequence.back.first-16)<=.5);
  ...
  const unchanged=await page.evaluate(()=>{const bar=document.querySelector('.sd-tabs'),tabs=[...bar.children],b=bar.getBoundingClientRect(),tab=tabs.find(t=>{const r=t.getBoundingClientRect();return r.left>=b.left&&r.right<=b.right;});activeTab=tab.dataset.tab;const x=bar.scrollLeft;renderModal();return Math.abs(document.querySelector('.sd-tabs').scrollLeft-x)<=.5;});
 ...
 ok('host nav/button margin and sizing cannot skew gutters',Math.abs(stressed.first.left-16)<=.5&&Math.abs(stressed.end.right-16)<=.5&&Math.abs(stressed.first.first-stressed.end.last)<=.5&&stressed.box==='border-box');await stress.evaluate(node=>node.remove());
 ...
 for(const family of['classic','editorial','glass']){
  await page.setViewportSize({width:393,height:900});await page.evaluate(async family=>{await setScene(family,family==='classic'?'dark':'light');document.querySelector('.sd-body').scrollTop=0;document.activeElement?.blur();},family);
  const path=fileURLToPath(new URL(`${family}-393.png`,qa));const windowBox=await page.locator('.sd-window').boundingBox();await page.screenshot({path,clip:{x:windowBox.x,y:windowBox.y,width:windowBox.width,height:Math.min(600,windowBox.height)},caret:'hide'});screenshots.push(path);
 }
