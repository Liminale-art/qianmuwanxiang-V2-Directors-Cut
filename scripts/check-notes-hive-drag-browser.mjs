...
async function mount(appearance = null) {
  await page.evaluate(({ handler, appearance }) => {
    ...
    window.fixture = { saves: 0, rendered: 0, opened: 0, closed: 0, notices: 0, lastPointer: null, noteSettings: { detached: false, position: null } };
    Object.assign(window, { notesFeatureSettings: () => fixture.noteSettings, detachedNotesGeometry: () => ({ width: 42, height: 48 }),
      saveSettings: () => fixture.saves++, renderFloatingNotes: () => fixture.rendered++,
...
async function measure() {
  return page.evaluate(() => {
    ...
    return { x: rect.x + rect.width / 2, y: rect.y + rect.height / 2, tx: matrix.m41, ty: matrix.m42,
      ...
      ink: style.color, edge: style.getPropertyValue('--sd-wheel-edge'), connected: button.isConnected, saves: fixture.saves, rendered: fixture.rendered, opened: fixture.opened, closed: fixture.closed,
...
function clean(result, { committed = false } = {}) {
  ...
  assert.equal(result.saves, committed ? 1 : 0); assert.equal(result.rendered, committed ? 1 : 0);
}
...
try {
  ...
  for (const appearance of [null, { theme: 'editorial', mode: 'light', accent: '#97324d' }, { theme: 'glass', mode: 'dark', accent: '#425fac' }]) {
    ...
    assert.equal(moved.capture, true); assert.equal(moved.ready, false); assert.equal(moved.saves, 0);
    ...
    checks.push(`${appearance?.theme || 'classic'}: mouse-follow feedback survives palette change and only successful release commits one synthetic setting`);
  }
  ...
  for (const cancellation of ['pointercancel', 'lostcapture', 'remove']) {
    ...
    assert.equal(end.saves, 0); assert.equal(end.rendered, 0); assert.equal(end.detached, false); assert.equal(end.opened, 0);
  ...
  for (const ending of ['touchEnd', 'touchCancel']) {
    ...
    moved = await measure(); assert.equal(moved.ready, true); near(moved.tx, 84, 'touch continues through recolor'); assert.equal(moved.saves, 0);
  ...
  console.log(JSON.stringify({ passed: checks.length, checks, errors, external, productionDataRead: false, productionWrites: false,
    scope: 'real CSS and detach handler, native mouse and CDP touch pointers; storage and opening use synthetic counters; physical-phone acceptance pending' }));
