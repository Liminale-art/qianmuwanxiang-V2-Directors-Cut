...
const allowed=new Set(['qianmu-prose-floor-tools.js','qianmu-text-collection-floor.js','qianmu-plain-text-range.js','qianmu-current-chat-source.js','qianmu-chat-file-target.js','qianmu-model-response.js','qianmu-llm-output.js','qianmu-portable-connection.js','qianmu-account-local-store.js','qianmu-storage-backup-view.js',...['floor','panel','source','context','session','request','messages','preferences','history-contract','history','history-runtime','storage'].map(name=>`qianmu-prose-assistant-${name}.js`)]);
...
try{
  ...
  await page.evaluate(async()=>{
    ...
    fixture.openHistory=async()=>{const {captureProseAssistantSource}=await import('./qianmu-prose-assistant-source.js'),{openProseAssistantHistory}=await import('./qianmu-prose-assistant-history-runtime.js'),source=await captureProseAssistantSource(fixture.source);try{const runtime=await openProseAssistantHistory({source,isCurrent:()=>fixture.live});return {runtime,source,close(){runtime.close();source.close();}};}catch(cause){source.close();throw cause;}};
  });
  ...
  assert.match(await page.locator('[data-pa-history]').textContent(),/尚未保存/);
  ...
  const sentAtReopen=await page.evaluate(()=>fixture.sent.length);await action('close').click();await page.evaluate(()=>fixture.open());assert.equal(await page.locator('[data-pa-turn]').count(),4);assert.equal(await page.evaluate(()=>fixture.sent.length),sentAtReopen);
  assert.match(await page.locator('[data-pa-turn="2"] small').textContent(),/未完成/);assert.match(await page.locator('[data-pa-turn="3"] small').textContent(),/已停止/);assert.equal(await page.locator('[data-pa-turn="3"] pre').textContent(),'半截');
  checks.push('normal close/reopen restores complete, failed and cancelled plaintext from actual isolated IndexedDB without repeating any model request');
  ...
  await page.getByLabel('助手API预设').selectOption('profile:chosen');await page.evaluate(()=>{fixture.nativePut=IDBObjectStore.prototype.put;IDBObjectStore.prototype.put=function(){throw new DOMException('private quota detail','QuotaExceededError');};});
  await question().fill('保存失败测试');await action('send').click();await action('retry-history').waitFor({state:'visible'});assert.equal(await action('send').isDisabled(),true);assert.equal(await page.locator('[data-pa-turn]').count(),1);
  assert.equal(await page.evaluate(async()=>{const h=await fixture.openHistory();try{return h.runtime.initialHistory().rows.length;}finally{h.close();}}),0);
  assert.doesNotMatch(await page.locator('[data-pa-history]').textContent(),/private quota/);await page.evaluate(()=>{fixture.confirm=false;});await action('close').click();assert.equal(await page.locator('dialog[open]').count(),1);await action('copy').click();
  const beforeRetry=await page.evaluate(()=>fixture.sent.length);await page.evaluate(()=>{IDBObjectStore.prototype.put=fixture.nativePut;});await action('retry-history').click();await page.waitForFunction(()=>document.querySelector('[data-pa-history]')?.textContent==='本机对话已保存');assert.equal(await page.evaluate(()=>fixture.sent.length),beforeRetry);
  await action('close').click();await page.evaluate(()=>fixture.open());assert.equal(await page.locator('[data-pa-turn]').count(),1);assert.match(await page.locator('[data-pa-turn] p').textContent(),/保存失败测试/);
  checks.push('quota failure preserves copyable reply, blocks unsafe next mutations and warns before close; explicit retry persists the same turn without another model call');
  await page.evaluate(()=>{fixture.confirm=true;IDBObjectStore.prototype.put=function(){throw new DOMException('quota','QuotaExceededError');};});await action('clear').click();await action('retry-history').waitFor({state:'visible'});assert.equal(await page.locator('[data-pa-turn]').count(),1);
  await page.evaluate(()=>{IDBObjectStore.prototype.put=fixture.nativePut;});await action('retry-history').click();await page.waitForFunction(()=>!document.querySelector('[data-pa-turn]'));
  checks.push('failed clear retains visible original dialogue until its exact empty snapshot receives a transaction acknowledgement');
  await page.getByLabel('助手API预设').selectOption('profile:chosen');await page.evaluate(()=>{fixture.mode='hold';});await question().fill('关闭时停止测试');await action('send').click();await page.waitForFunction(()=>document.querySelector('[data-pa-turn] pre')?.textContent==='半截');
  await action('close').click();await page.waitForFunction(()=>!document.querySelector('dialog'));await page.evaluate(()=>fixture.open());assert.equal(await page.locator('[data-pa-turn] pre').textContent(),'半截');assert.match(await page.locator('[data-pa-turn] small').textContent(),/已停止/);assert.equal(await page.evaluate(()=>fixture.sent.at(-1).signal.aborted),true);
  checks.push('explicit close stops an in-flight reply and confirms its cancelled partial history before leaving; reopen does not resume generation');
  await page.evaluate(async()=>{const h=await fixture.openHistory();try{await h.runtime.save({key:h.source.key,busy:false,rows:[]});}finally{h.close();}fixture.mode='ok';});await page.getByLabel('助手API预设').selectOption('profile:chosen');await question().fill('另一页面清空后的新问题');await action('send').click();
  await page.waitForFunction(()=>document.querySelector('[data-pa-history]')?.textContent.includes('另一页面'));assert.equal(await action('retry-history').isVisible(),false);assert.equal(await page.locator('[data-pa-turn]').count(),2);
  assert.equal(await page.evaluate(async()=>{const h=await fixture.openHistory();try{return h.runtime.initialHistory().rows.length;}finally{h.close();}}),0);await action('close').click();await page.evaluate(()=>fixture.open());assert.equal(await page.locator('[data-pa-turn]').count(),0);
  checks.push('another page clear is never overwritten by this panel; conflict keeps new text copyable, and acknowledged discard/reopen reads the actual empty history');
  await action('close').click();await page.evaluate(async()=>{fixture.nativeGet=IDBObjectStore.prototype.get;IDBObjectStore.prototype.get=function(){throw Error('private read failure');};await fixture.open();});assert.equal(await action('send').isDisabled(),true);assert.equal(await action('clear').isDisabled(),true);assert.equal(await action('retry-history').textContent(),'重新读取');
  await page.evaluate(()=>{IDBObjectStore.prototype.get=fixture.nativeGet;});await action('retry-history').click();await page.waitForFunction(()=>document.querySelector('[data-pa-history]')?.textContent.includes('已读取'));
  checks.push('history read failure offers explicit retry and never replaces unknown stored history with an empty writable session');
  ...
  const storage=await page.evaluate(async()=>{const summary=await fixture.tools.assistantStorageSummary(()=>true),{renderStorageBackupSection}=await import('./qianmu-storage-backup-view.js');
    const container=document.createElement('section');container.className='sd-card';container.id='storage-fixture';container.style.cssText='box-sizing:border-box;width:100%;max-width:100%;';container.innerHTML=renderStorageBackupSection(null,n=>n+' B',{data:{assistantStorage:summary}});document.body.append(container);return summary;});
  assert.equal(storage.namespace,'st-user:fixture');assert.equal(storage.status,'ready');assert.equal(storage.count,0);assert.ok(storage.markers>=1&&storage.bytes>0);assert.match(await page.locator('.sd-storage-assistant-summary').textContent(),/清空版本标记.*不含未保存回复/);
  for(const width of [320,1280]){await page.setViewportSize({width,height:820});const fit=await page.locator('.sd-storage-assistant-summary').evaluate(node=>({scroll:node.scrollWidth,client:node.clientWidth}));assert.ok(fit.scroll<=fit.client+1,JSON.stringify(fit));}
  assert.equal(await page.evaluate(()=>fixture.sent.length),sentBefore);checks.push('shared facade lazily reads actual account history without an open chat/panel, and storage text remains bounded on narrow/wide viewports without any model request');
  assert.equal(external,0);assert.deepEqual(errors,[]);console.log(JSON.stringify({checks,count:checks.length,externalRequests:external,pageErrors:errors,productionWrites:false,persistence:'real isolated IndexedDB with synthetic host/model responses; not ST sync'},null,2));
