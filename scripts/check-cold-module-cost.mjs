 ...
 for(const scenario of ['identity-via-admission','identity-direct','comfy-library-cold','vibe-foreground-only','vibe-during-idle']){
  ...
  const result=await page.evaluate(async scenario=>{
   ...
   if(scenario==='comfy-library-cold'){
    const module=await import('/qianmu-comfy-library-view.js?v=1.59.387');return {elapsedMs:performance.now()-start,exports:Object.keys(module).length};
   }
   const {loadLocalChunk}=await import('/qianmu-feature-runtime.js?v=1.59.387');
   ...
   const clicked=performance.now(),module=await loadLocalChunk('./qianmu-vibe-library-view.js?v=1.59.387');
