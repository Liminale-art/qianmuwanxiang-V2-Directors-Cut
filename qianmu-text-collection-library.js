...
export async function openTextCollectionLibrary({parent,resolveNamespace,isCurrent,headers,confirm,copy,download}={}){
  ...
  let closed=false,busy=false,record=null,operation=null,pendingText=null,pageIndex=0,cursors=[null],nextCursor=null,searchValue='',resolve,session,outbox;
  ...
  function stop(){
    if(closed)return;closed=true;outbox?.close();session?.close();observer.disconnect();view.removeEventListener('pagehide',stop);
  ...
  async function discard(){return !dirty()||await confirm(pendingText===editor.value?'离开编辑':'放弃修改',pendingText===editor.value?'修改尚未确认同步，已保留以便重试。继续离开吗？':'此收藏有未保存的文字，放弃当前修改吗？')===true;}
  ...
  async function openRecord(id){
    ...
    q('[data-collection-pages]').hidden=true;q('[data-collection-detail-actions]').hidden=false;status.textContent='';
  }
  async function click(event){
    ...
    await run(async()=>{
      ...
      if(action==='save'){
        ...
        catch(cause){if(cause?.localSaved===true){pendingText=operation.request.text;status.textContent=`保存未完成，当前内容已保留。${/^text_collection_sync_/.test(cause?.code||'')?String(cause.message).slice(0,160):'请重试。'}`;return;}throw cause;}
        if(!current())return;
