...
export async function openTextCollectionLibrary({parent,resolveNamespace,isCurrent,headers,confirm,copy}={}){
  ...
  let closed=false,busy=false,record=null,operation=null,pageIndex=0,cursors=[null],nextCursor=null,resolve;
  ...
  async function discard(){return !dirty()||await confirm('放弃修改','此收藏有未保存的文字，放弃修改吗？')===true;}
  ...
  async function loadPage(index=0,reset=false){
    const cursor=reset?null:cursors[index];const page=await session.list({cursor,limit:50});if(!current())return;
    if(reset)cursors=[null];pageIndex=index;nextCursor=page.nextCursor;record=null;operation=null;editor.value='';
  ...
  async function openRecord(id){
    ...
    record=result.record;operation=null;editor.value=record.text;editor.readOnly=true;
    q('[data-collection-title]').textContent=textCollectionListLabel(record);list.hidden=true;detail.hidden=false;
  ...
  async function click(event){
    ...
    await run(async()=>{
      ...
      if(action==='save'){
        ...
        const ack=await operation.submit();if(!current())return;
        record=textCollectionRecord({...record,text:operation.request.text,revision:ack.revision,updatedAt:ack.updatedAt});operation=null;editor.readOnly=true;status.textContent='收藏修改已保存';return;
      }
      if(action==='delete'){
        ...
        catch{await session.guard();list.replaceChildren();list.hidden=false;detail.hidden=true;q('[data-collection-pages]').hidden=false;q('[data-collection-detail-actions]').hidden=true;
