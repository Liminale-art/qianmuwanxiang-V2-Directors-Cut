...
export async function checkComfyReadiness(input, { fetchImpl = globalThis.fetch, signal, timeoutMs = 20000 } = {}) {
  ...
  try {
    for (const name of classes) {
      ...
      catch (error) { if (controller.signal.aborted || /^(?:comfy_readiness_|comfy_transport_|comfy_targets_)/.test(error?.code || '')) throw error; throw fail('transport', '无法读取 Comfy 节点清单，请核对网络、浏览器跨域限制或 ST 转发方式'); }
      if (!response.ok) {
