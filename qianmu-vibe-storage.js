...
export function createVibeStorageOperations({store,encodings,locks=globalThis.navigator?.locks}){
  async function read(namespace){
    const local=await store.inventory(namespace),receipts=await encodings.list(namespace),fingerprint=await hash([namespace,local,receipts]);
    ...
    return {local,view:{version:1,namespace,fingerprint,items,receiptCount:receipts.length,receiptBytes:receipts.reduce((sum,row)=>sum+size(row),0),
...
export function createVibeStorageController({actions,confirm=async()=>false,onClose,icons=()=>{},isCurrent=()=>true}){
  ...
  function render(){
    ...
    host.innerHTML=`<section class="sd-vibe-storage">
      ...
      <p>文件 ${rows.length} / ${snapshot.usage.countLimit} · ${bytes(snapshot.usage.bytes+snapshot.usage.previewBytes)} / ${bytes(snapshot.usage.limit)}<br>记录 ${snapshot.receiptCount} 条，其中未决 ${snapshot.pendingCount} 条。计值为内容大小，非浏览器实际磁盘占用或剩余空间。</p>
