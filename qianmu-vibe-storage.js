...
export function createVibeStorageActions({namespace,call,guard,items=()=>[]}){
  ...
  return {
    ...
    async aggregate(snapshot,id){
      ...
      const result=await call('aggregate-prepare',{namespace,id});await guard();return snapshot.persistence==='st-account-file'?{...result,persistence:snapshot.persistence}:result;
    },
    async exportAggregate(plan,confirm){
      ...
      const {id,fingerprint,file}=plan;await guard();const yes=await confirm('导出全档汇总',`这会导出${plan.persistence==='st-account-file'?'同账户ST目录':'当前账户、本设备'}已保存的同原图档位；名称、预览和默认值沿用所选基准文件。\n汇总是一个新素材，不会修改原文件、替换旧引用、清理记录或重新编码。\n它不能代替原文件备份来恢复旧镜头引用；清理前仍须另行导出原文件。确认下载？`);
...
export function createVibeStorageController({actions,confirm=async()=>false,onClose,icons=()=>{},isCurrent=()=>true,createPreservation=null}){
  ...
  function renderAggregate(){
    ...
    host.innerHTML=`<section class="sd-vibe-review sd-vibe-aggregate"><header><h3>同原图档位汇总</h3><button class="sd-icon-btn sd-vibe-aggregate-close" type="button" aria-label="返回文件空间"><i class="fa-solid fa-xmark"></i></button></header>
      ...
      <p class="sd-vibe-review-file-info">基准：${escape(report.name)}<br>${report.fileCount} 份同原图文件 · ${report.variantCount} 个唯一档位 · ${report.duplicateCount} 个重复档位已去重<br>${aggregate.persistence==='st-account-file'?'当前ST账户活跃目录':'本设备当前账户'}已保存的原图文件，不含已移出目录或来源不明的纯编码。名称、预览、日期和默认值沿用基准文件。</p>
  ...
  function render(){
    ...
    const native=snapshot?.persistence==='st-account-file',rows=snapshot?.items||[],total=rows.reduce((sum,row)=>sum+row.bytes+row.previewBytes,0)+(snapshot?.receiptBytes||0)+(snapshot?.metadata?.bytes||0)+(snapshot?.retained?.bytes||0);
    ...
    if(snapshot&&!busy&&!message&&!rows.length)host.querySelector('.sd-vibe-storage-list').innerHTML=`<p class="sd-vibe-empty" role="status">${native?'当前ST账户活跃目录':'当前设备与账户'}暂无 Vibe 文件</p>`;
    ...
    host.querySelector('.sd-vibe-storage-remove')?.addEventListener('click',run(async active=>{
      ...
      snapshot=null;selected.clear();message=result.retained?`已移出 ${result.removed} 份文件；原件继续保留，未释放磁盘空间。明确重新导入同一原文件可恢复。`:`已清理 ${result.removed} 份文件，约 ${bytes(result.bytes)}；仅可通过重新导入原备份恢复。`;
