...
export function parseStoryboardPackageText(text){
  ...
  if(!object(payload)||payload.type!=='qianmu-storyboard'||payload.version!==7||!object(payload.settings)||!object(payload.chat))fail('请选择新版 v7 分镜包；旧版恢复仍使用原入口');
  if(payload.credentialsIncluded!==false)fail('分镜包未声明排除凭据，请先重新安全导出');
  return payload;
}
...
function validateMedia(payload){
  ...
  for(const row of media){
    ...
    if(!['image/png','image/jpeg','image/webp'].includes(row.mime)||typeof row.b64!=='string'||!row.b64.length||row.b64.length>32*1024*1024||row.b64.length%4||!/^[A-Za-z0-9+/]*={0,2}$/.test(row.b64))fail('分镜媒体须为 24 MiB 以内的 PNG、JPEG 或 WebP');
    ...
    if(mime!==row.mime)fail('分镜媒体类型与实际内容不符');
  }
}

// Read-only input gate. A parsed packet is not permission to apply settings or replay jobs.
export async function inspectStoryboardPackageFile(file){
  ...
  const payload=parseStoryboardPackageText(text);validateMedia(payload);
  const inspection=await inspectStoryboardVibePackage(payload);
