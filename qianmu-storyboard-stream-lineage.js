import {normalizeStoryboardContinuationLinks,storyboardContinuationSignature as signature} from './qianmu-storyboard-continuation-proof.js?v=1.59.232';
import {hasStoryboardStreamReference,normalizeStoryboardStreamReference,storyboardStreamGeneration} from './qianmu-storyboard-stream-reference.js?v=1.59.232';
...
export function createStoryboardStreamLineage(current,message,links,namespace){
  ...
  const endpoints=new Set([signature(endpoint)]),keys=new Set([current.messageKey]);let following=null,count=0;
  while(true){
    ...
    endpoint=row.from;following=row;endpoints.add(signature(endpoint));keys.add(endpoint.messageKey);
  }
  return Object.freeze({namespace,matches(ref){
