...
self.addEventListener('message', async event => {
  ...
  try {
    ...
    if (message.action === 'open' && !session) {
      ...
      const workflowStore = createComfyWorkflowStore({native}), poolStore = createComfyPoolStore({native}), characterStore = createCharacterArchiveStore({native}), vibe = createVibeAssetStore({native:native?{...native,onProgress:()=>self.postMessage({id,operation,progress:++progress})}:false}), journal = createStoryboardPackageJournal({native}),carrierStore=createBundleCarrierStore({native:native?{...native,onProgress:()=>self.postMessage({id,operation,progress:++progress})}:false});
