...
export function createChatGalleryDigest({capture=true}={}){
  if(typeof capture!=='boolean')fail('图库摘要捕获方式无效');
  ...
  return Object.freeze({
    append(raw){
      ...
      try{
        const record=capture?galleryDigestRecord(raw):galleryRecordFingerprintText(raw);
        const encoded=capture?record.encoded:utf8.encode(record.text);bytes+=record.bytes+(count?1:0);nodes+=record.nodes;
        if(count>=LIMIT.records||bytes>LIMIT.bytes||nodes>LIMIT.nodes)fail('图库超过分批核验范围，未裁剪原件');
        if(count)hash.update(utf8.encode(','));hash.update(encoded);count++;return record;
...
export function chatGalleryDigest(rows){
  if(rows===undefined)return null;const length=galleryDigestRows(rows),state=createChatGalleryDigest({capture:false});
...
export async function scanChatGallery(rows,{guard=()=>{},yieldWork=async()=>{},visit}={}){
  if(visit!==undefined&&typeof visit!=='function')fail('图库记录接收器无效');
  const length=galleryDigestRows(rows),state=createChatGalleryDigest({capture:typeof visit==='function'});let workBytes=0;
