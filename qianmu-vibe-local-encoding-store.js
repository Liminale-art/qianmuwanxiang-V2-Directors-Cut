...
import {captureVibeReceiptOriginal,vibeReceiptEvidenceText} from './qianmu-vibe-receipt-original.js';
...
import {hash,account,attempt,fail,digest,key,object,equalReceipt,ARCHIVE_BYTES,receiptBytes,normalizeVibeEncodingReceipt,VIBE_ENCODING_RECEIPT_LIMIT,VIBE_ENCODING_ARCHIVE_LIMIT,validateVibeEncodingIdentity,matchesVibeServiceDelivery} from './qianmu-vibe-encoding-contract.js';

// Durable metadata only, with no expiry or implicit retry. A crashed submitting receipt remains uncertain.
export function createLocalVibeEncodingStore({indexedDB=globalThis.indexedDB,keyRange=globalThis.IDBKeyRange,dbName='qianmu-vibe-encodings',timeoutMs=8000,now=Date.now}={}){
  ...
  return Object.freeze({
    // Internal native mirror: exact old and new evidence, in one local atomic
    // transaction. The caller preserves the old evidence remotely FIRST.
    async materialize(namespace,input,expected){
      const next=(await captureVibeReceiptOriginal(input,namespace)).snapshot,cacheKey=next.receipt.cacheKey;
      const before=expected===null?null:(await captureVibeReceiptOriginal(expected,namespace)).snapshot;
      if(before&&before.receipt.cacheKey!==cacheKey)throw fail('identity','费用镜像原记录不符');
      const signature=value=>vibeReceiptEvidenceText(value),id=key(namespace,cacheKey),rowBytes=receiptBytes(next.receipt),segmentBytes=next.segments.reduce((n,r)=>n+receiptBytes(r),0),reviewCount=next.segments.reduce((n,r)=>n+r.reviews.length,0);
      return transaction('readwrite',(table,read,set,archive,usage,segments,reviewUsage)=>read(table.get(id),current=>read(archive.get(id),archived=>{
        if(current&&archived)throw fail('corrupt','当前和历史记录重复，未同步');
        read(segments.index('receipt').getAll(keyRange.only([namespace,cacheKey]),VIBE_REVIEW_SEGMENT_LIMIT+1),oldSegments=>{
          const old=current||archived,actual=old?{namespace,section:current?'current':'archived',receipt:old,segments:oldSegments}:null;
          if(signature(actual)!==signature(before)||!old&&oldSegments.length)throw fail('changed','本机费用记录已变化，未覆盖');
          readUsage(archive,usage,read,namespace,totals=>readReviewUsage(segments,reviewUsage,read,namespace,history=>read(table.index('namespace').count(keyRange.only(namespace)),count=>{
            const archivedCount=totals.count-Number(Boolean(archived))+Number(next.section==='archived'),archivedBytes=totals.bytes-(archived?receiptBytes(archived):0)+(next.section==='archived'?rowBytes:0);
            const totalSegments=history.count-oldSegments.length+next.segments.length,totalBytes=history.bytes-oldSegments.reduce((n,r)=>n+receiptBytes(r),0)+segmentBytes,totalReviews=history.reviews-oldSegments.reduce((n,r)=>n+r.reviews.length,0)+reviewCount;
            if(count-Number(Boolean(current))+Number(next.section==='current')>VIBE_ENCODING_RECEIPT_LIMIT||archivedCount>VIBE_ENCODING_ARCHIVE_LIMIT||archivedBytes>ARCHIVE_BYTES||totalSegments>4096||totalBytes>ARCHIVE_BYTES)throw fail('capacity','本机费用镜像超过原有容量，未裁剪');
            if([archivedBytes,totalBytes,totalReviews].some(n=>n<0))throw fail('corrupt','费用镜像计值不符');
            table.delete(id);archive.delete(id);(next.section==='current'?table:archive).put(next.receipt);
            for(const row of oldSegments)segments.delete(row.key);for(const row of next.segments)segments.put(row);
            usage.put({namespace,count:archivedCount,bytes:archivedBytes});reviewUsage.put({namespace,count:totalSegments,bytes:totalBytes,reviews:totalReviews});set(next);
          })));
        });
      })));
    },
