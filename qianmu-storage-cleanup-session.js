...
export function createStorageCleanupSession({owner, scope, epoch, activity = () => ({}), notify = () => {}, watchView = () => null}) {
  let active = null;
  ...
  return {
    ...
    begin(root) {
      ...
      let invalid = false, watcher;
      try { watcher = watchView(root); }
      catch (_) { notify('清理页面已关闭或变化，请重新打开后再试。', 'warning'); return null; }
      const token = {
        current(view = true) {
          if (invalid) return false;
          try {
            if (view) watcher?.check();
        ...
        release() { if (active === token) { active = null; watcher?.release(); } },
      };
      active = token;
      return token;
    },
