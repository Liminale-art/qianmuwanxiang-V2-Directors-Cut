...
export function createDirectorDecision(candidateValue = {}, packetValue = {}, options = {}) {
  ...
  const decision = normalizeDirectorDecision({
    ...
    source: { candidateId: candidate.candidateId, ledgerEntryId: candidate.entryId, packetId: packet.packetId, eventId: packet.eventId },
    approval: { mode: 'explicit', approvedAt, revision: 1 },
    outputs,
    lanes: {
      visual: { duty: visual.duty, shotPattern: visual.shotPattern, subject: visual.subject, description: visual.description },
