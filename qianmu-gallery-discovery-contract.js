...
export function galleryDiscoveryRequest(raw){
  ...
  if(!exact(value,['version','expectedAccount','limit','cursor'])||value.version!==1||typeof value.expectedAccount!=='string'||!/^st-user:[a-f0-9]{64}$/.test(value.expectedAccount)
    ||!Number.isSafeInteger(value.limit)||value.limit<1||value.limit>GALLERY_DISCOVERY_LIMITS.page)fail('图库发现只接受账户核对和受限分页');
  const cursor=value.cursor;
  if(cursor!==null&&(!exact(cursor,['version','account','stamp','after'])||cursor.version!==1||cursor.account!==value.expectedAccount||!hash(cursor.stamp)||!hash(cursor.after)))fail('图库发现续页已失效');
  return value;
}
export async function galleryDiscoveryResponse(raw,{namespace,request}={}){
  ...
  if(!exact(value,['ok','version','expectedAccount','entries','nextCursor','proof'])||value.ok!==true||value.version!==1||value.expectedAccount!==expected.expectedAccount
  ...
  for(const entry of value.entries){
    ...
    if(checked.scope.namespace!==namespace||await galleryArchiveSourceSlot(checked.scope,checked.sourceReceipt)!==`gallery-source-${entry.key}`)fail('图库发现包含其他账户或错误来源');
