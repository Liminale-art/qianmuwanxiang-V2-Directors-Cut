...
export async function openFocusDialogue({document,host,library,guard,choices,binding,escape:esc,icons,confirm,onClose=()=>{}}){
  ...
  portal.innerHTML='<section class="sd-focus-library sd-focus-dialogue" role="dialog" aria-modal="true" aria-label="自定义台词库"><header><h3>自定义台词库</h3><button type="button" class="sd-icon-btn" data-action="close" aria-label="关闭"><i class="fa-solid fa-xmark"></i></button></header><div class="sd-focus-library-body"></div><p role="status" class="sd-focus-library-status"></p></section>';
  ...
  let closed=false,busy=false,dirty=false,data,editing=null,folder=binding().characterKey||'';const selected=new Set();
  ...
  function render(){
    editing=null;dirty=false;title('自定义台词库');const visible=data.rows.filter(row=>!folder||row.characterKey===folder);
    ...
    body.innerHTML=`<div class="sd-focus-dialogue-folders"><select class="text_pole" data-field="folder" aria-label="选择角色"><option value="">全部角色</option>${folders().map(([key,name])=>`<option value="${esc(key)}" ${key===folder?'selected':''}>${esc(name)}</option>`).join('')}</select>${button('new','新建')}</div>
      ...
      <div class="sd-focus-library-list">${visible.map(row=>`<article><label class="sd-focus-dialogue-check"><input type="checkbox" data-select="${esc(row.id)}" aria-label="选择台词：${esc(row.text.slice(0,60))}" ${selected.has(row.id)?'checked':''}></label><button type="button" class="sd-focus-library-item" data-action="edit" data-id="${esc(row.id)}"><b>${esc(row.text)}</b><span>${esc(row.speaker)} · ${row.moments.map(key=>moments[key]).join(' / ')}</span></button></article>`).join('')}</div>`;syncSelection();icons(portal);
  }
  ...
  function edit(row){
    ...
    title(editing.speaker||'角色');
    body.innerHTML=`<label>语音内容<textarea class="text_pole" rows="4" maxlength="2000" data-field="text">${esc(editing.text)}</textarea></label><div class="sd-focus-library-moments">${Object.entries(moments).map(([key,label])=>`<label><input type="checkbox" data-moment="${key}" ${editing.moments.includes(key)?'checked':''}>${label}</label>`).join('')}</div><div class="sd-focus-dialogue-actions"><button type="button" class="sd-icon-btn" data-action="back" title="返回列表" aria-label="返回列表"><i class="fa-solid fa-list"></i></button>${button('save','保存')}${editing.id?button('remove','删除'):''}</div>`;
    icons(portal);
