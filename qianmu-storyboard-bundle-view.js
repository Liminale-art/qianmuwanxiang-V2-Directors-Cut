...
export function renderStoryboardBundleReview(view) {
  const p = view.preview, offset = view.page * 24, conflicts = p?.conflicts || [], bindings = p?.bindingReview || [], subjects = p?.subjectReview || [];
  const pages = Math.max(1, Math.ceil(Math.max(conflicts.length, bindings.length, subjects.length) / 24));
  const ready = p?.ready && !p.needsRecheck && view.environmentReviewed && (!bindings.length || view.bindingsReviewed) && (!subjects.length || view.subjectsReviewed);
...
export function openStoryboardBundleReview({ parent, fileName, connect, paintIcons = () => {} }) {
  ...
  const view = { fileName, page: 0, preview: null, busy: true, notice: '', result: null, environmentReviewed: false, bindingsReviewed: false, subjectsReviewed: false };
  ...
  async function run(action) {
    ...
    try {
      if (action === 'preview') { view.environmentReviewed = false; view.bindingsReviewed = false; view.subjectsReviewed = false; view.preview = await session.preview(choices); }
      if (action === 'choose') view.preview = await session.choose(choices);
      if (action === 'restore') {
        view.result = await session.restore(view.preview, { confirmed: true, environmentReviewed: view.environmentReviewed, bindingsReviewed: view.bindingsReviewed, subjectsReviewed: view.subjectsReviewed });
      ...
      view.environmentReviewed = false; view.bindingsReviewed = false; view.subjectsReviewed = false;
  ...
  dialog.addEventListener('change', event => {
    ...
    if (field.matches('[data-bundle-environment]')) { focusChoice = null; view.environmentReviewed = field.checked; draw(); }
      ...
      view.environmentReviewed = false; view.bindingsReviewed = false; view.subjectsReviewed = false; view.preview = { ...view.preview, ready: false, needsRecheck: true, planDigest: '' }; void run('choose');
    }
  });
