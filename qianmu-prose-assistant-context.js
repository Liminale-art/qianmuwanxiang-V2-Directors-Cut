import {captureProseAssistantSource,PROSE_ASSISTANT_SOURCE_LIMIT} from './qianmu-prose-assistant-source.js';
...
export async function captureProseAssistantContext(options={}){
  ...
  if(!Number.isSafeInteger(previousFloors)||previousFloors<0||previousFloors>limits.maxPreviousFloors)fail('正文助手前文范围无效');
  const source=await captureProseAssistantSource(options),tracked=[];let closed=false;
  const close=()=>{closed=true;tracked.length=0;source.close();};
  function assertCurrent(){
    try{
      if(closed)fail('正文助手引用已关闭');source.assertCurrent();
  ...
  try{
    const previous=[],omitted=[];let previousCharacters=0,historyCharacters=0;const first=Math.max(0,source.reference.floor-previousFloors);
    // Inspect at most the explicit preceding window, never search older floors to fill gaps.
    for(let floor=source.reference.floor-1;floor>=first;floor--){
    ...
    if(history!==undefined){
      if(!history||history.key!==source.key||!Array.isArray(history.turns)||history.turns.length>10000)fail('正文助手历史不属于当前会话或超出容量');
      historyCount=history.turns.length;
      for(let index=historyCount-1;index>=Math.max(0,historyCount-limits.historyPairs);index--){
        const pair=history.turns[index];if(!pair||!validText(pair.user)||!pair.user.trim()||!validText(pair.assistant)||!pair.assistant.trim())fail('正文助手历史须为完整的问答');
    ...
    const summary=Object.freeze({referenceFloor:source.reference.floor,referenceMode:source.reference.mode,referenceCharacters:source.reference.text.length,
      previousWindow:Object.freeze({start:first,end:source.reference.floor}),previousIncluded:Object.freeze(previous.map(row=>row.floor)),previousOmitted:Object.freeze(omitted),previousCharacters,
      historyPairs:pairs.length,historyOmitted:historyCount-pairs.length,historyCharacters});
    return Object.freeze({key:source.key,scope:source.scope,reference:source.reference,previous:Object.freeze(previous),history:Object.freeze(pairs),summary,guard,assertCurrent,close});
  }catch(cause){close();throw cause;}
}
