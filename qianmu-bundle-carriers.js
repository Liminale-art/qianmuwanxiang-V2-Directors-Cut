...
export async function captureBundleCarriers({namespace,store,mappings,guard=async()=>{},isCurrent=()=>true}){
  ...
  const before=await inventory(),baseline=await digest(before),mappingFiles=new Map(),rawFiles=new Map(),entries=[],originals=[];
  ...
  for(const head of before.originals){
    await check();const blob=await store.loadOriginal(namespace,head.sha256,{guard:check,isCurrent});await original(blob,head,namespace,mappings.index,check);
    const reused=mappingFiles.get(head.sha256),entryId=reused?reused.id:`carrier-original:${head.sha256}`;
    originals.push({...head,entryId});rawFiles.set(head.sha256,blob);if(!reused)entries.push({id:entryId,file:blob});
  }
  for(const head of before.heads){
    ...
    await verifyBundleCarrierMembers(proof,{load:({sha256})=>rawFiles.get(sha256)||null,guard:check});entries.push({id:bundleCarrierEntryId(head),file:file(proof)});
  }
...
export async function inspectBundleCarriers(opened,{mappings,guard=async()=>{}}={}){
  ...
  const rawFiles=new Map();for(const row of index.originals){
    ...
    const part=await opened.read(entryId);await original(part.file,head,namespace,mappings?.index,guard);rawFiles.set(head.sha256,part.file);
  }
  for(const head of index.heads){
    ...
    await verifyBundleCarrierMembers(proof,{load:({sha256})=>rawFiles.get(sha256)||null,guard});
  }
