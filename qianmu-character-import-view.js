...
export function renderCharacterImportReview(review,{escape,icon,busy=false,error=''}={}){
  ...
  return `<div class="sd-character-library sd-character-restore" aria-busy="${busy}"><fieldset ${busy?'disabled':''}>
    ...
    <section class="sd-card"><div class="sd-storyboard-card-body sd-character-restore-items">${rows.map((row,i)=>`<label>
      ...
      <select class="text_pole" data-archive-legacy-choice="${page*24+i}"><option value="" ${!choices[row.key]?'selected':''}>请选择</option><option value="current" ${choices[row.key]==='current'?'selected':''}>保留当前</option><option value="source" ${choices[row.key]==='source'?'selected':''} ${row.deleted||row.categoryConflict||row.alias||row.missing?'disabled':''}>采用旧端</option></select>
