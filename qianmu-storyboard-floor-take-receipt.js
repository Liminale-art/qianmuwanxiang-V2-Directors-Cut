...
function receipt(value) {
  if(!object(value)||value.version!==1||!text(value.id,160)||!text(value.chatKey,512)||!text(value.messageKey,160)
...
export function mergeStoryboardFloorTakeReceipts(history,takes=[]) {
  ...
  for(const take of takes){
    const next=receipt({...take,baselineTaskIds:take.baselineTaskIds||[]}),key=scope(next),previous=byScope.get(key);
...
export function storyboardFloorTakeReceiptSupersedes(record,receipts) {
  ...
  return receipts.some(take=>{
    ...
    return Boolean(own?.version===1&&scope(own)===scope(take)&&integer(own.startedAt)&&text(own.id,160)&&order(own,take)<0);
  });
}
