...
import {createEnsembleLibraryEditor} from './qianmu-ensemble-editor.js?v=1.59.379';
...
export function renderEnsembleSchemeList(view){
  ...
  const rows=(view.library?.schemes||[]).filter(row=>!row.archived&&(!needle||[row.name,row.description,...row.tags].some(text=>text.toLocaleLowerCase().includes(needle))));
  if(!view.ready)return `<p class="sd-ensemble-empty">${view.busy?'正在读取方案库…':'暂未读取，点击刷新重试。'}</p>`;
  if(!rows.length)return `<p class="sd-ensemble-empty">${needle?'没有匹配的风格方案':'还没有风格方案'}</p>`;
  return rows.map(row=>`<article class="sd-ensemble-scheme ${selected.has(row.id)?'is-selected':''}"><div class="sd-ensemble-scheme-copy"><b>${escape(row.name)}</b>${row.description?`<p>${escape(row.description)}</p>`:''}<div class="sd-ensemble-tags">${row.tags.map(tag=>`<span>${escape(tag)}</span>`).join('')}</div></div><div class="sd-ensemble-scheme-actions">${icon('edit','编辑 '+row.name,'pencil-simple',view.busy,`data-id="${escape(row.id)}"`)}<button type="button" class="sd-ensemble-select" data-ensemble-action="toggle" data-id="${escape(row.id)}" aria-label="本聊天启用 ${escape(row.name)}" aria-pressed="${selected.has(row.id)}" ${view.busy||!view.chatKey?'disabled':''}>${selected.has(row.id)?'已选':'选用'}</button></div></article>`).join('');
}
function form(view){
  ...
  const configured=route?.available!==false&&route;
  return `<section class="sd-ensemble-editor"><header><b>${view.library.schemes.some(row=>row.id===draft.id)?'编辑风格方案':'新建风格方案'}</b>${icon('cancel','返回方案库','x',view.busy)}</header><div class="sd-ensemble-fields"><label><span>风格名</span><input class="text_pole" data-ensemble-field="name" maxlength="80" value="${escape(draft.name)}" ${view.busy?'disabled':''}></label><label><span>适用画面</span><textarea class="text_pole" data-ensemble-field="description" maxlength="800" rows="3" ${view.busy?'disabled':''}>${escape(draft.description)}</textarea></label><label><span>风格标签</span><input class="text_pole" data-ensemble-field="tagText" maxlength="2000" placeholder="用逗号分隔" value="${escape(draft.tagText)}" ${view.busy?'disabled':''}></label><div class="sd-ensemble-binding-fields"><label><span>生成方式</span><button type="button" data-ensemble-action="configure" ${view.busy||!view.canConfigure?'disabled':''}>${escape(configured?(route.label||[route.providerLabel,route.name].filter(Boolean).join(' · ')):'选择模型或工作流')}</button></label>${route?.artistCapable?`<label><span>画师方案</span><select class="text_pole" data-ensemble-field="artistPresetId" ${view.busy?'disabled':''}>${option('','当前画师',!artistId)}${artistId&&!view.artists.some(row=>row.id===artistId)?option(artistId,'请重新选择',true,true):''}${view.artists.map(row=>option(row.id,row.name,row.id===artistId)).join('')}</select></label>`:''}</div></div><footer>${view.library.schemes.some(row=>row.id===draft.id)?icon('remove','删除方案','trash',view.busy,`data-id="${escape(draft.id)}"`):'<span></span>'}<button type="button" class="sd-ensemble-save" data-ensemble-action="save" ${view.busy||view.needsRefresh?'disabled':''}>${glyph('floppy-disk')}<span>保存</span></button></footer></section>`;
}
export function renderEnsembleLibrary(view){
  const blocked=view.busy||!view.ready,choice=view.selection;
  return `<section class="sd-ensemble-library" aria-label="镜组风格方案" aria-busy="${view.busy}">${view.draft?'':`<div class="sd-ensemble-chat"><button type="button" data-ensemble-action="enabled" class="sd-ensemble-choice" aria-pressed="${Boolean(choice?.enabled)}" ${blocked||!view.chatKey?'disabled':''}>启用镜组</button><button type="button" data-ensemble-action="lock" class="sd-ensemble-choice" aria-pressed="${choice?.styleLock!==false}" ${blocked||!view.chatKey?'disabled':''}>连续风格锁</button></div><div class="sd-ensemble-toolbar"><input type="search" class="text_pole" data-ensemble-search aria-label="搜索风格方案" placeholder="搜索风格方案" value="${escape(view.search)}" ${blocked?'disabled':''}>${icon('refresh','刷新方案库','arrows-clockwise',view.busy)}${icon('add','新建风格方案','plus',blocked)}</div>`}<div class="sd-ensemble-status" role="status" aria-live="polite">${escape(view.error||view.message||'')}</div>${view.draft?form(view):`<div data-ensemble-list>${renderEnsembleSchemeList(view)}</div>`}</section>`;
}
...
export function mountEnsembleLibrary(root,options){
  ...
  const click=event=>{const button=event.target.closest?.('[data-ensemble-action]');if(!button||!root.contains(button)||button.disabled)return;
    ...
    try{let result;const view=model.snapshot();
      ...
      else if(action==='remove')result=model.remove(id);else if(action==='configure')result=model.configure();else if(action==='refresh')result=model.refresh();
  ...
  const change=event=>{try{const field=event.target.dataset?.ensembleField;if(field&&event.target.tagName==='SELECT')model.setField(field,event.target.value);}catch(error){report(error);}};
