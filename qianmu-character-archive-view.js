  ...
  if(view.restoring){
    ...
    return `<div class="sd-character-library sd-character-restore" aria-busy="${Boolean(view.busy)}"><fieldset ${disabled}>
      ...
      ${conflicts.length?`<section class="sd-card"><summary><b>逐项确认冲突 · ${conflicts.length}</b></summary><div class="sd-storyboard-card-body sd-character-restore-items">${conflicts.slice(offset,limit).map((row,index)=>`<label>
        ...
        <select class="text_pole" data-archive-restore-decision="${offset+index}"><option value="" ${!row.choice?'selected':''}>请选择</option><option value="local" ${row.choice==='local'?'selected':''}>保留当前</option><option value="incoming" ${row.choice==='incoming'?'selected':''}>使用备份</option></select></label>`).join('')}</div></section>`:''}
...
export function createCharacterArchiveController({resolveNamespace,getContext,getScope,isCurrent=()=>true,onIcons=()=>{},identity,notify=()=>{},confirm=async()=>false,download,saveReference,onUserAliases,requestHeaders=()=>({}),
  ...
  const exportDocument=doc=>{const data=exportCharacterArchive(doc);download(new Blob([JSON.stringify(data,null,2)],{type:'application/json'}),`${doc.name.replace(/[\\/:*?"<>|\u0000-\u001f]/g,'_')}.character.qianmu.json`);if(data.comfyOmitted)notify('已导出文字档案，参考图与旧工作流绑定未包含在内','info');else if(data.referenceOmitted)notify('已导出文字档案，参考图请在目标环境重新选择','info');};
  ...
  function bind() {
    ...
    host.querySelector('[data-archive-file]')?.addEventListener('change',event=>{const file=event.target.files?.[0];if(!file)return;void run(async guard=>{
      if(file.size>128*1024)throw Error('档案文件须小于 128 KB');const imported=importCharacterArchive(await file.text());await guard();edit(imported.document);view.draft.dirty=true;if(imported.comfyOmitted)notify('文字档案已载入，旧工作流绑定未导入；参考图请重新选择','info');else if(imported.referenceOmitted)notify('文字档案已载入，请重新选择参考图','info');
    });});
