import { assistantStorageRequest, assistantStorageResponse, ASSISTANT_STORAGE_LIMITS as LIMIT } from './qianmu-assistant-storage-contract.js';
import { proseAssistantAccountForNamespace } from './qianmu-prose-assistant-source.js';
import { parseBoundedJson } from './qianmu-json-input.js';

const BASE = '/api/plugins/qianmu-tts/assistant/storage';
...
export async function collectAssistantNativeStorage({ resolveNamespace, isCurrent, headers, fetchImpl = globalThis.fetch, timeoutMs = 6000, signal } = {}) {
    ...
    const work = async () => {
        ...
        const request = assistantStorageRequest({ version: 1, expectedAccount }), provided = new Headers(await headers?.());
        ...
        try {
            ...
            if (!/^application\/json\b/i.test(response.headers?.get?.('content-type') || '') || Number(response.headers.get('content-length')) > LIMIT.responseBytes) {
            ...
            while (true) { const part = await reader.read(); await guard(); if (part.done) break; bytes += part.value.byteLength; if (bytes > LIMIT.responseBytes) throw unavailable(); text += decoder.decode(part.value, { stream: true }); }
            text += decoder.decode(); const value = parseBoundedJson(text, { maxBytes: LIMIT.responseBytes, maxDepth: 5, maxNodes: 80, label: '助手文件盘点' });
            if (!response.ok || value?.ok !== true) { notice = Object.hasOwn(messages, value?.code) ? messages[value.code] : ''; throw unavailable(); }
            const result = assistantStorageResponse(value, expectedAccount); await guard(); return Object.freeze({ namespace, status: 'ready', ...result });
        } finally {
