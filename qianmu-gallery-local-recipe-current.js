...
export async function readCurrentGalleryLocalRecipe({record,getContext,epoch,account,createStorage=createConfiguredStAccountStorage,readLocal=readLegacyGalleryRecipe,timeoutMs=12000}={}){
  ...
  const work=async()=>{
    ...
    if(!galleryLocalRecipeReference(scope,copy))throw Error('旧本机配方缺少完整内容引用，未猜补');
    let row;try{row=await readLocal(copy.snapshotRef);}catch{row=null;}await guard();
    ...
    if(result.state!=='available')throw Error('原配方仍只在旧设备，尚未保全；未使用当前设置替代');return result.snapshot;
  };
