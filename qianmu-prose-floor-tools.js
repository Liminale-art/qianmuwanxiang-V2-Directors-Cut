...
import {createProseAssistantFloorTools} from './qianmu-prose-assistant-floor.js?v=1.59.388';
...
export function createProseFloorTools(options){
  ...
  const assistant=createProseAssistantFloorTools({...options,assistantHistoryFactory:async input=>{await renames?.settled(input.source);return options.assistantHistoryFactory?options.assistantHistoryFactory(input):(await loadLocalChunk('./qianmu-prose-assistant-native.js?v=1.59.388')).openNativeProseAssistantHistory(input);}});
