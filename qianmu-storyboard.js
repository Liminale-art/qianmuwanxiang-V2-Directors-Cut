...
export function normalizeStoryboardState(value) {
  ...
  state.schemaVersion = STORYBOARD_SCHEMA_VERSION; state.enabled = Boolean(state.enabled); state.automation = normalizeStoryboardAutomation(state.automation); state.directorBridge = { worldSideShotsEnabled: Boolean(state.directorBridge?.worldSideShotsEnabled) }; state.view = ['create', 'characters', 'assets', 'artists', 'presets', 'workflows', 'gallery', 'logs'].includes(state.view) ? state.view : 'create'; state.workspaceView = ['workbench', 'assets', 'artists', 'presets', 'gallery', 'logs'].includes(state.workspaceView) ? state.workspaceView : 'workbench'; state.logFilter = ['all', 'success', 'failed'].includes(state.logFilter) ? state.logFilter : 'all';
  ...
  if (state.view === 'workflows' && state.source !== 'comfy') state.view = 'create';
