...
import {hasStoryboardStreamReference,normalizeStoryboardStreamReference,resolveStoryboardStreamReference,normalizeStoryboardStreamFinalCapture,storyboardStreamBudgetReference} from './qianmu-storyboard-stream-reference.js?v=1.59.389';
import {normalizeWorldAutomaticApproval} from './qianmu-world-automatic-approval.js?v=1.59.389';
import {normalizeStoryboardStreamMoment} from './qianmu-storyboard-stream-moment.js?v=1.59.224';
import {normalizeStoryboardStreamAttempt} from './qianmu-storyboard-stream-attempt.js?v=1.59.389';
import {readStoryboardContinuationLinks} from './qianmu-storyboard-continuation-proof.js?v=1.59.389';
import {resolveStoryboardOrdinaryContinuation} from './qianmu-storyboard-ordinary-continuation.js?v=1.59.389';
...
import {normalizeStoryboardFloorTake} from './qianmu-storyboard-floor-take.js?v=1.59.389';
export {normalizeStoryboardFloorTake,createStoryboardCaptureReservation,bindStoryboardFloorTakeJobs,applyStoryboardFloorTakeToJob,storyboardFloorTakeInitialInline,saveStoryboardFloorTakes,settleStoryboardFloorTakes,pruneStoryboardRetakeGallery} from './qianmu-storyboard-floor-take.js?v=1.59.389';
...
export function projectStoryboardProtocolParameters(providerId, parameters, connection = {}) {
  ...
  // Empty form controls mean "not configured". In the NAI gateway Number('')
  // would otherwise turn them into seed 0, steps 1 or scale 0.
  if (binding.protocol === IMAGE_NATIVE_PROTOCOLS[providerId]) return providerId === 'novel'
    ? Object.fromEntries(Object.entries(obj(parameters) ? parameters : {}).filter(([, value]) => value !== '')) : parameters;
  const compatibility = normalizeOpenAIImageCompatibility(connection.compatibility);
...
const legacyProfile = () => ({ loaded: false, model: '', sampler: '', scheduler: '', width: '', height: '', ratio: '1:1', count: '', steps: '', cfg: '', seed: '', comfyUrl: '', comfyWorkflow: '', comfyWorkflowNotice: '', comfyOutputNodeId: '', openaiStyle: '', openaiQuality: '', openaiBackground: '', openaiOutputFormat: '', imageSize: '', watermark: false, seedreamGuidanceScale: '', seedreamSequential: false, googleEnhance: false, novelCfgRescale: '', novelSm: false, novelSmDyn: false, novelDecrisper: false, novelVarietyBoost: false });
...
export function normalizeStoryboardComfyPreparation(value) {
  ...
  try {
    ...
    return {version:1,pool,styleLock:value.styleLock,scope,chatKey:str(value.chatKey,512),floor:value.floor,target:value.target,
      ...
      prompt:str(value.prompt,24000),negative:str(value.negative,12000),shotType:str(value.shotType,60),tags:uniqueStrings(value.tags,30,80)};
  }catch(_){return null;}
}
...
export function storyboardRecipeRecordMetadata(record) {
  return {production:storyboardProductionContext(record),decision:storyboardDirectorDecisionSnapshot(record)};
}
...
export function normalizeStoryboardAutomation(value) {
  ...
  // Retire the duplicate extraction switch without re-enabling image spending
  // for an old user who disabled it. Drop the old field after this one-way read.
  return { autoGenerate: raw.autoCapture === undefined ? autoGenerate : Boolean(raw.autoCapture) && autoGenerate, streamEnabled: raw.streamEnabled === true };
}
...
function shotPlans(value, state = {}) {
  const normalized = (Array.isArray(value) ? value : []).filter(obj).map((plan) => {
    ...
    return {
      ...
      ...(Object.hasOwn(plan,'streamAttempt')?{streamAttempt:normalizeStoryboardStreamAttempt(plan.streamAttempt)}:{}),
      ...(Object.hasOwn(plan,'ensembleRecovery')?{ensembleRecovery:retainEnsembleRecoveryRecord(plan.ensembleRecovery)}:{}),
...
export function normalizeStoryboardParameterProfile(value, providerId) {
  ...
  if(providerId!=='comfy'&&Object.hasOwn(p,'artDirection'))base.artDirection=retainStoryboardArtDirection(p.artDirection);
  ...
  if (providerId === 'comfy') {
    ...
    if(Object.hasOwn(p,'comfyConsoleUrl')){try{base.comfyConsoleUrl=normalizeRunningHubConsoleUrl(p.comfyConsoleUrl);}catch(_){base.comfyConsoleUrl='[invalid]';}}
...
function legacyLogs(value) {
  ...
  for(const log of normalized){
    ...
    if(raw?.kind==='prompt_compiler'){log.kind='prompt_compiler';if(raw.promptOrigin==='world')log.promptOrigin='world';log.source='compiler';log.snapshot=null;log.submissionState='not_submitted';log.params={};log.recordId='';log.recordIds=[];}
  }
