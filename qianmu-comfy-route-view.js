...
export function renderComfyRoutePicker({ heads, selectedId, versions = [], selectedRevision = '', roles = false, useReferences = false, hasReferences = false, message = '' }) {
  return `<div class="sd-comfy-route-picker"><h3>固定工作流分工</h3>
    ...
    <label class="sd-comfy-route-check"><input type="checkbox" data-comfy-route-roles ${roles ? 'checked' : ''}><span>使用角色库中匹配此版本的实现</span></label>
    ...
    <small>本分工使用该版本的参数与提示补充，不改变当前工作台。保留本分工已绑参考图；换图不匹配时须先移除。本地核对不等于远端执行验证，确认不会生成。</small>
    <p role="status">${escape(message || (!heads.length ? '请先在 Comfy 工作流库保存方案。' : ''))}</p></div>`;
}

export async function openComfyRoutePicker({ context, namespace, binding, roles = false, hasReferences = false, guard = async () => {}, createStore = createComfyWorkflowStore }) {
  ...
  try {
    ...
    for (;;) {
      ...
      wrap.innerHTML = renderComfyRoutePicker({ heads, selectedId, versions, selectedRevision, roles, useReferences, hasReferences, message });
      ...
      roles = wrap.querySelector('[data-comfy-route-roles]').checked; useReferences = hasReferences && wrap.querySelector('[data-comfy-route-references]').checked;
      ...
      return { recipe, roles, useReferences };
    }
  } finally { store.close(); }
}
